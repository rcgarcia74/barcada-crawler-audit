# Session 26 prompt — scope picked at Phase 1
# (S25 closed Candidate J; S26 chooses from carry-forwards A/B/D/E/H
#  + optional new follow-on: ADLSCostJournal live Azure smoke)

**Drafted at Session 25 close (2026-05-24).** Mirrors the
S20/S21/S22/S23/S24/S25 prompt structure. Scope-agnostic at
Phases 0/1; scope-specific design gates at Phase 2 per chosen
candidate. Strict 7-phase ordering with halt-on-mismatch
preserved.

This prompt should be invoked from `~/Downloads/session-26-prompt.md`
(operator-mirrored) or directly from
`~/crawler-audit/SESSION_26_PROMPT.md`. Re-read it on session open.
...
Session 25 closed Candidate J (2 commits) and left repo HEAD at
`aed7873` without a new tag (1.TAG=defer):

- **Candidate J shipped fully (S25)** — abfss:// CostJournal
  Phase 5 promotion. The production
  `_open_cost_journal_for_worker` now dispatches abfss:// URLs
  through `open_journal` → `ADLSCostJournal` (full Azure-blob
  backend with native ETag preconditions). Tests against an
  in-memory `DummyBlobBackend` (Q-J.3 (c)); the production code
  path against real Azure has NOT been smoke-tested.
...
- **Optional new follow-on (Candidate K)** — ADLSCostJournal live
  Azure smoke. S25 shipped against `DummyBlobBackend` in-memory
  (Q-J.3 (c)); no production code path was ever exercised against
  real Azure. A small operator-driven smoke test
  (write_initial + try_update + read against a sandbox container)
  would close the mock-vs-prod divergence risk. NOT a session-
  scope blocker on its own; can be folded into Candidate B's
  verification if B is chosen.

Operator chooses at Phase 1 which candidate Session 26 ships. Each
candidate has its own Phase 2 design-gate template.

**Sessions 13-25 precedent:** this session does NOT modify the
...
at `aed7873`). Does NOT modify production code under
...
prompt before Session 26 starts, walk each flagged item against
on-disk reality at workspace HEAD `8e6d836` and repo HEAD
`aed7873` (or whatever HEAD the operator's machine carries),
BEFORE applying any change. Per S19-S25 pattern (LESSONS
...
skipped 1 WRONG-PREMISE + 1 defer-to-implementation. Review remains
...
halts catch hidden scope expansion (per S22-S25 "Implicit-
...
```
# Workspace at Session 26 start (Session 25 close-out: primary +
# 1 follow-up pinning the S26 anchor SHA, all pushed):
git -C ~/crawler-audit rev-parse HEAD
# Expect: 8e6d836 (S25 close-out follow-up: pin S26 Phase 0
# workspace anchor SHA) OR a later commit if additional workspace
# doc edits landed post-S25-close. If N commits ahead, verify each
# prior commit's subject via `git log --oneline 8e6d836..HEAD`
# against expected prompt-finalization / doc-edit patterns;
# surface the SHA delta and request authorization to proceed if
# anything is unexpected. (S20-S25 precedent: operator authorized
# continuation when 2-3 extra workspace commits were the
# strengthened prompts themselves.)

# Repo at Session 25 final commit:
git -C /Users/administrator/projects/barcada-scraper rev-parse HEAD
# Expect: aed7873 (WA2.W8.adls-promotion-worker-loop) — the
# canonical end-of-S25 ship SHA.
#
# Tolerated delta: operator-side eval_data labeling commits between
# S25 close and S26 open are expected (Sessions 8-25 precedent).
...
# aed7873..HEAD; surface any non-eval_data delta for operator
# authorization before continuing.
```

### Step 0.2 — Tags (no change from S25 close)

```
git -C /Users/administrator/projects/barcada-scraper tag -l | sort
# Expect 10 tags (unchanged from S22/S23/S24/S25 close; S25 did
# not place a tag):
#   baseline-v0
...
### Step 0.4 — Fixture counts (no change from S25 close)
...
### Step 0.5 — Test-suite baseline (S26 canonical)
...
#
# Pinned in SESSION_LOG.md "Canonical S25-close baseline" block.
# Re-verified post-S25-close-out at HEAD aed7873: 964 passed in
# ~55s. The 964 count is invariant under operator-side eval_data
# commits between S25 and S26 (eval_data is not in the invocation).
```
...
**Narrower baselines** (valid for S26 candidates that don't
...
### Step 0.9 — S25-shipped public API stability (any-candidate prereq)

```
# Verify the S25 deliverables match what landed at S25 close. A
# change to any of these between S25 close and S26 open would
# invalidate carry-forward candidate assumptions. Run unconditionally
# at every S26 cold start.
...
# routes abfss:// URLs to ADLSCostJournal (NOT to LocalFSCostJournal
...
config = WorkerConfig(
    ...
    worker_id='test',
    # Sentinel GUID; the _AzureBlobBackend constructor is patched out
    # so no Azure SDK / credential touch happens.
...
## Required workspace reading (Session 26 first 10 minutes)

In this order:

1. **`~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md`** — full
   handoff state at S25 close. Lists scope candidates (A/B/D/E/H
   carry-forward + optional K) with prerequisites + estimated scope.
   The S26 scope choice at Phase 1 picks from these.

2. **`~/crawler-audit/SESSION_LOG.md`** Session 25 entry — what
   landed during Candidate J (2 commits). The 3 forward-applicable
   patterns folded into LESSONS at S25 close.

3. **`~/crawler-audit/LESSONS.md`** — 3 new sections folded at S25
   close, at end of file. Locate via
   `grep -n '^## .*(S25 folding)' LESSONS.md`. Especially:
   - "Phase 2 source-verify drives option-set design, not just gates"
     — for any candidate touching a new SDK / external system,
     verify the underlying facts that the prompt's options assume
     BEFORE the AskUserQuestion batch.
   - "Q-J.8 explicit allowlist may be incomplete; HALT-and-extend
     pattern" — Out-of-scope + Q-*-style explicit allowlists are
     defenses against silent scope expansion, NOT exhaustive
     inventories. Grep for same-shape items at Phase 0; HALT-and-
     extend if surfaced mid-Phase-3.
   - "Local imports defeat module-attribute monkeypatch" — applies
     to any candidate writing tests that mock orchestrator
     dependencies; spot-check import-site (top-of-module vs
     deferred-inside-function) BEFORE assuming the textbook
     monkeypatch pattern.
...
9. **`docs/CRAWLING_POLICY.md`** at `fdc8a7a` — only if Candidate H
   (doc tightening) is chosen.

10. **`src/barcada_scraper/orchestrator/worker_loop.py`** lines
    2014-2057 — only if Candidate B / K touches the
    `_open_cost_journal_for_worker` dispatch. Body modified at
    S25 SHA `aed7873` (abfss:// guard removed; abfss:// dispatch
...
Session 26 AND operator has not authorized explicit placeholder
choices.
...
**Prerequisites:**
- Operator authorization at S26 Phase 1 for a W5.X-prefix commit
...
### Candidate K — ADLSCostJournal live Azure smoke (NEW; optional)
...
before-asking-discipline]]` AND per S22-S25 "Plan-vs-reality at
...
authorized (per S22-S25 "Implicit-authorization HALT for src/-
...
per Candidate B's auth) OR to any S19-S25 deliverable — surface
as a design-gate sub-question before patching.

### If Candidate A (barcada-drift)

Carry-forward from S22-S25 prompts (unchanged).
...
- **NEW Q-B.6** (S26 addition): does the retrofit verify against
  abfss:// CostJournal (folding Candidate K in) or LocalFSCostJournal-
  only? Default LocalFSCostJournal-only unless operator opts in.
...
- **Q-SHARED.1 Commit shape**: per-module (S18-S25 default;
...
fully before starting the next. Per S22-S25 precedent.
...
the format-fix into the commit (per S19-S25 pattern).
...
sessions (Sessions 8-25 precedent).
...
```
Phase 3 start                  : 964  (Session 25 close baseline)
...
canonical S26 baseline for any candidate that touches the
orchestrator sub-surface OR the journal subsurface (Candidates B
+ K definitely; A possibly; D/E/H likely not). Whichever baseline
is bound at Phase 1, hold it consistent across ALL Phase 3
commits in S26 — do not switch mid-session.
...
S20+S22 precedent: operator chose (a). S21/S23/S24/S25 did not
need this protocol at Phase 4 push.
...
- Append Session 26 entry to `~/crawler-audit/SESSION_LOG.md`
  including a **Canonical S26-close baseline** block with the
  exact pytest invocation + verified test count (per S22-S25
  LESSONS "Pin the S26 baseline for Phase 0 Step 0.5"; without
  this, S27 Phase 0 will infer from the "Combined headline"
  bullet and potentially HALT on accounting-mismatch).
- Refill `~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md` for
  Session 27 — explicitly pin the S27 Phase 0 workspace anchor
  SHA per S21-S25 post-audit pattern (LESSONS "Workspace HEAD
  ...
  canonical baseline test count discovered during audit; S23/S24/S25
  needed 1).
...
Note: drafting the next-session prompt (`SESSION_27_PROMPT.md`)
is NOT a built-in Phase 6 step. Per S20→S21, S21→S22, S22→S23,
S23→S24, S24→S25, S25→S26 precedent, prompt-drafting is an
operator-commissioned activity between sessions — not always-on
close-out work. If the operator asks for it explicitly at S26
close, draft it as a separate follow-up; otherwise leave for the
next session to either operator-commission or scope out at S27
open.
...
5. NEW Q-B.6: if abfss:// verification opted in, ADLSCostJournal
...
- **S1.** Combined suite at session close: existing 964 baseline
  ...
  (H), the baseline is the chosen narrower count + N new.
...
- **S4.** Regression-protection checklist held (see "Out-of-scope"
  below). In particular: ALL S21-S25 deliverables stay at the
  ...
  worker_loop_persistence tests stay 12/12 green. The total stays
  at 964 (or grows) unless a Q-* AskUserQuestion explicitly
...
**S25 deliverables (Session 25 W A.2 sub-surfaces — NEW for S26
lock-list):**
...
- The S25 invoker-site comment update at SHA `aed7873`. The
  locked S24 wiring block (lines 2241-2243) is unchanged; only
  the adjacent comment was updated.
...
- All workstream tags at their placed SHAs (10 tags as of S25
...
- All of `eval_data/` — labeling-workstream territory; per-row WIP
  edits across sessions are expected and unstaged (Sessions 8-25
  precedent). S22→S25 inter-session operator-side eval_data
...
- `src/barcada_scraper/` — locked unless Phase 2 design-gate
  ...
  helper body). Those authorizations do NOT extend to other
  src/ modules or to further modifications of the authorized
  files beyond their landed S21-S25 shapes.
...
## Verify-before-asking discipline (strict rule from S19-S25)
...
LESSONS-folded discoveries from S22-S25 worth re-applying:
...
- **Phase 2 source-verify drives option-set design** (S25 LESSONS):
  ...
  option set first, then ask. Cases at S25: Q-J.4 pathlib `//`-
  collapse drove the option set; Q-J.6 Azure 412 semantics made
  it a fact-check, not a design gate.

- **Implicit-authorization HALT** (S22-S25 LESSONS): Q-*
  answers may require touching files NOT in S26 Out-of-scope's
...
- **Source-verify line numbers per Phase 3 commit** (S23
  ...
  lines 2014-2057 at S25 close. Re-Explore at session-current
  HEAD before drafting commit edits; line numbers shift as
  commits land.

---

## Commit hygiene (per LESSONS + S19/S20/S21/S22/S23/S24/S25 additions)
...
- For S24/S25-test modifications: explicit Phase 2 authorization
  required; document the modification scope explicitly in the
  commit body (per S22-S25 LESSONS "Implicit-authorization HALT"
  pattern).
- Workspace close-out (Phase 6) lands as its own commit at session
  ...
  for the next session (S21-S25 pattern).

---

## Context-window awareness

S25 ran across 2 commits + Phase 2 source-verification + 1 mid-
Phase-3 HALT-and-extend + Phase 6 close-out, well within context.
S26 budget per scope:
...
12. Verify-before-asking summary: any source-verification
    findings surfaced.
13. Outstanding items for Session 27.
14. Tags state at S26 close.
...
All S25 amendments (close-out commit `2b05cdd` + follow-up
`8e6d836`) plus the S25 post-close verification have been folded
directly into this prompt — S26 does not need a separate
amendment file:

- **S25 close-out corrections** folded into Step 0.1 (workspace
  anchor `8e6d836`; repo anchor `aed7873` with operator-eval_data-
  commit tolerance), Step 0.5 (Canonical S25-close baseline 964
  pinned with 16-path invocation + sub-totals), Step 0.9 (S24-
  shipped + S25-shipped public API stability check covers the
  ADLSCostJournal full backend + the body-only abfss:// dispatch
  in worker_loop's helper).
- **3 LESSONS sections from S25 close** referenced where they
  apply: Phase 2 source-verify drives option-set design (Phase 2
  source-verify discipline); Q-J.8 explicit allowlist may be
  incomplete (Phase 3 same-shape regression HALT-and-extend);
  Local imports defeat module-attribute monkeypatch (any candidate
  writing tests that mock orchestrator dependencies).

If new amendments arise pre-S26 open, walk them per the reviewer-
