...
- **Last commit SHA at Session 22 CLOSE: [TO-BE-RECORDED post-
  commit]**. S23 prompt's Phase 0 Step 0.1 MUST anchor workspace
  expectation to that SHA, NOT chain forward from `190e75b`
  (which would spuriously surface a 1-commit-delta HALT on S23
  open). Workspace HEAD delta tolerance (LESSONS) handles N
