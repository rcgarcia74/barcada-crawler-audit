# Session 37 prompt — scope picked at Phase 1
# (S36 closed a NEW candidate at repo 25c3696 — fresh live ADLS
#  coverage for page_storage.write_pages via adlfs [the Stage-2
#  pages.parquet write surface; same fsspec/adlfs stack as the S35
#  parquet ShardWriter, but a DIFFERENT auth seam — write_pages takes no
#  storage_options and resolves credentials from the environment]. That
#  candidate is now CLOSED. S37 enters with the SAME near-empty
#  warm-candidate queue as S33/S34/S35/S36: A blocked / D operator-led /
#  E exhausted. No new carry-forward was introduced by S36 — but the
#  remaining uncovered adlfs write surfaces [PartitionedShardWriter Hive
#  path, prompt_logger] plus the lease/SAS cost-journal paths are still
#  explicit fresh-candidate space.)

**Drafted at Session 36 close (2026-06-02), operator-commissioned.**
Mirrors the S20/.../S35/S36 prompt structure. Scope-agnostic at
...
This prompt should be invoked from `~/crawler-audit/SESSION_37_PROMPT.md`.
...
Engineering session. Repo HEAD at `25c3696`
(WA2.W8.adls-live-page-storage; S36). Workspace HEAD at the S36
close-out chain `cfb6851` (primary close-out) + `fedb096` (anchor-pin)
+ any later prompt-drafting commit (see Step 0.1). **Workstream 0 fully
closed at S27 via the `workstream-0-end` tag at `a1c5636`**.

Carry-forward candidates entering S37:

- **barcada-drift (Candidate A)** — deferred since S20; still needs
  4 AI/ML decisions per CLASSIFICATION_ADJACENT_PLAN.md §Item 8 AND
  2+ `canary_runs/*.parquet` files. **S36 re-confirmed empirically**
  that the launchd installer has NOT been run (no plist in
  `~/Library/LaunchAgents/`; 0 `canary_runs` parquets on disk). Same
  state as S29-S36 close. **Remains BLOCKED.**
...
- **page_storage adlfs live coverage (S36 candidate) — CLOSED at S36**
  (`25c3696`, port 10003). The Stage-2 `pages.parquet` write is now
  live-tested against Azurite via adlfs with env-var auth
  (`test_page_storage_adls_azurite.py`). **NOT a carry-forward.**

**No new carry-forward introduced by S36.** The S36 deliverable was a
single commit (1 new live test file); the canonical 970 test count is
unchanged.

**Scope-availability note (IMPORTANT):** with A blocked, D
operator-led, E exhausted, and the S35+S36 candidates now closed, S37
may have **no clearly-actionable engineering scope** unless the operator
...
E, or (d) commissions a NEW candidate. **The remaining fresh-candidate
space** — the OTHER uncovered adlfs write surfaces:
  - `output/parquet_writer.py::PartitionedShardWriter` (the Hive
    `write_to_dataset(filesystem=fs)` path — a DIFFERENT pyarrow code
    path than the S35-covered `ParquetWriter`/`ShardWriter`, still
    uncovered live);
  - `classifier/llm/prompt_logger.py` (fsspec `wb` single object, JSONL);
...
**S36 auth-seam lesson — apply before scoping ANY adlfs candidate:**
read the production call site's SIGNATURE first. S35's `ShardWriter`
takes an explicit `storage_options=` kwarg; S36's `page_storage.write_pages`
took NONE and resolved auth from `AZURE_STORAGE_CONNECTION_STRING` in the
environment. `PartitionedShardWriter` and `prompt_logger` may differ
again — verify, do not assume.

Operator chooses at Phase 1 which candidate Session 37 ships.
Each candidate has its own Phase 2 design-gate template.

**Sessions 13-36 precedent:** this session does NOT modify the
...
Does NOT modify the Session 19-36 deliverables. Does NOT modify the
...
(`test_page_storage_adls_azurite.py`).
...
S37 candidates: Candidate A would add a NEW `barcada-drift` module
...
before Session 37 starts, walk each flagged item against on-disk
reality at the workspace HEAD and repo HEAD `25c3696`, BEFORE applying
any change. Per S19-S36 pattern:
...
was sequential) is exactly the kind of discrepancy this protocol
exists to surface. **S35 anchor:** the build-time validation spike
caught the adlfs `makedirs`-no-op gotcha BEFORE the test was committed.
**S36 anchor:** the operator-flagged "forced env-var auth" challenge was
resolved at SOURCE (the public signature has no credential param + the
prod caller uses the same signature → env is the only channel) BEFORE
committing the coverage claim. Apply the same discipline: validate any
external-service interaction AND any auth-seam claim against the real
...
# Expect: fedb096 (S36 anchor-pin) OR cfb6851 (S36 primary close-out)
# OR this S37 prompt-drafting commit succeeding it OR a later commit if
# additional workspace doc edits / prompt revisions landed. If N commits
# ahead of fedb096, verify each via `git log --oneline fedb096..HEAD`
# against expected prompt-finalization / doc-edit patterns; surface the
# SHA delta and request authorization if anything is unexpected. (S20-S36
# precedent: operator authorized continuation when 1-4 extra workspace
# commits were the strengthened prompts themselves.)
#
# NOTE: SESSION_36_PROMPT.md carries an uncommitted operator-side edit
# (a Step 0.10 section) left unstaged at S36 close. It may still be in
# the working tree at S37 open — tolerate it; it is operator territory.

git -C /Users/administrator/projects/barcada-scraper rev-parse HEAD
# Expect: 25c3696 (S36 WA2.W8.adls-live-page-storage).
#
# Tolerated delta: operator-side eval_data labeling commits between
# S36 close and S37 open (Sessions 8-36 precedent). Verify via
# `git show --stat <sha>` for every commit in 25c3696..HEAD that each
# is strictly within eval_data/* (NO src/barcada_scraper/*, NO tests/*
# [INCLUDING the four Azurite live-test files], NO .github/*
...
### Step 0.2 — Tags (13 expected; unchanged from S30-S36 close)

```
git -C /Users/administrator/projects/barcada-scraper tag -l | sort
# Expect 13 tags (UNCHANGED; S36 placed no tag per its 1.TAG = defer).
...
# at S34, S35 AND S36 despite four live-ADLS surfaces landing. If S37
# declares W A.2 closed, that annotated tag remains appropriate (1.TAG).
...
# S29-S36 did NOT touch this surface.
```

### Step 0.4 — Fixture counts (use the Python rglob pattern, NOT bare find)

**UNCHANGED FROM S33/S34/S35/S36**: S36 shipped a live test file, NOT a
...
### Step 0.5 — Test-suite baseline (S37 canonical)
...
#
# Sub-totals (16 paths; ALL identical to S27-S36 close — ALL FOUR
...
#
# Pinned in SESSION_LOG.md "Canonical S36-close baseline" block.
```
...
Phase 3 commits in S37. The cumulative-test-count gate never lets it
...
# S35 sub-surface (parquet writer hermetic suite — unchanged):
.venv/bin/python -m pytest tests/test_parquet_writer.py -q              # expect 33

# S36 sub-surface (page_storage hermetic suite — the file:// guard the
# S36 live test complements; unchanged by S36):
.venv/bin/python -m pytest tests/classifier/page_acquisition/test_page_storage.py -q  # expect 13
```

### Step 0.9 — S25-S36 invariants + cassette-dir + S33/S34/S35/S36 live-test presence

**Coverage note**: equals S36's Step 0.9 plus a NEW check for the S36
...
`cat >…<<'PYEOF'` heredocs.** Empirically re-confirmed S33-S36: the
...
# (a2) S35 parquet writer public API unchanged (inline-safe):
.venv/bin/python -c "
import inspect
...
# test depends on these public symbols — assert they remain, AND that
# write_pages STILL takes no credential/storage_options param [the auth
# seam is env-resolved; if a future change adds a storage_options kwarg,
# the S36 test's env-var contract must be re-examined]):
.venv/bin/python -c "
import inspect
...
#     tool; run via .venv/bin/python (re-use S36's check body verbatim).
...
#     tool (hook blocks inline ast.parse). Re-use S36's check_s28_ast body.

# (e) S29 K-b smoke script existence + import — own Bash call (secrets).
#     Re-use S36's check_s29_smoke body via the Write tool.

# (f) S31+S32 cassettes (10) + S33 Azurite primitive + S34 race test +
#     S34 CI workflow + S35 parquet adlfs test + S36 page_storage adlfs
#     test. Stage check_s37_deliverables.py via the Write tool and run it:
```

Stage the following as `check_s37_deliverables.py` via the Write tool
...
qtext = pq.read_text()
assert '@pytest.mark.live' in qtext, 'S35 test lost its live marker'
assert '_BLOB_PORT = 10002' in qtext, 'S35 fixture lost its distinct port'
assert 'barcada-azurite-parquet' in qtext, 'S35 fixture lost its distinct container name'
assert 'ShardWriter' in qtext and 'abfss://' in qtext, 'S35 test lost its ShardWriter/abfss drive'
assert 'AzureBlobFileSystem' in qtext, 'S35 test lost its teeth'
assert '--skipApiVersionCheck' in qtext, 'S35 fixture lost the baked version-skew flag'
...
stext = ps.read_text()
assert '@pytest.mark.live' in stext, 'S36 test lost its live marker'
assert '_BLOB_PORT = 10003' in stext, 'S36 fixture lost its distinct port (collides with S33/S34/S35)'
assert 'barcada-azurite-pages' in stext, 'S36 fixture lost its distinct container name'
assert 'write_pages' in stext and 'abfss://' in stext, 'S36 test lost its write_pages/abfss drive'
assert 'AZURE_STORAGE_CONNECTION_STRING' in stext, 'S36 test lost its env-var auth seam'
assert 'AzureBlobFileSystem' in stext, 'S36 test lost its teeth (negative-control assertion)'
assert '--skipApiVersionCheck' in stext, 'S36 fixture lost the baked version-skew flag'
print('OK (f) S31+S32 cassettes (10) + S33 primitive + S34 race+CI + S35 parquet + S36 page_storage adlfs tests present (posture intact)')
```

If any of 0.1-0.9 fail, HALT before doing any work.

**Optional Docker-gated check (NOT a halt condition)**: if Docker is
available locally, the operator MAY confirm ALL FOUR live tests still
pass + coexist:
```
.venv/bin/python -m pytest -m live tests/classifier/pipeline/ -q
# Expect: 4 passed, 209 deselected (needs Docker + the Azurite image;
# SKIPS cleanly if absent). Ports S33=10000 / S34=10001 / S35=10002 /
# S36=10003. Informational; Docker is NOT required for S37 unless the
# chosen scope is new live-test work.
```
...
surface (e.g. the S36 page_storage leg vs the 13 hermetic
`test_page_storage.py` tests), confirm the new test ADDS coverage rather
than duplicating/replacing the hermetic guard.

---

## Required workspace reading (Session 37 first 10 minutes)

In this order:

1. **`~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md`** — full handoff
   state at S36 close. Lists scope candidates (A/D/E) with
   prerequisites + the named fresh-candidate adlfs surfaces. The S35+S36
   candidates are CLOSED; Candidate E is EXHAUSTED at 30. Note the
   explicit near-empty-warm-queue flag.

2. **`~/crawler-audit/SESSION_LOG.md`** Session 36 entry — the
   page_storage auth-seam finding (env-var vs storage_options); the
   negative-control teeth check; the build-time spike +
   clear_instance_cache note; the 970 baseline re-pin.

3. **`~/crawler-audit/LESSONS.md`** — re-read sections relevant to the
   chosen candidate:
   - **Mandatory regardless of candidate**: the S36 fold "Survey the
     AUTH seam, not just the client stack" + the S35 fold "adlfs is a
     SEPARATE ADLS write stack from the azure-storage-blob SDK" + the
     S34 fold "A carve-out claim must be verified against the test BODY".
   - **If a NEW live-service candidate**: the S33 fold (live-emulator
     ...
     FRESH handle; the `makedirs`-no-op → `mkdir` gotcha; DISTINCT port —
     S33=10000, S34=10001, S35=10002, S36=10003, so use 10004+) + the S36
     fold (read the call-site signature to find the auth seam;
     clear_instance_cache before a URL-only resolution) + S30
     posture-validation + the S24/S29/S30/S33/S34/S35/S36
     public-API-only pattern.
   - **If Candidate A**: S29 prereq-audit fold + S22-S36
...
6. For a NEW adlfs-surface candidate, the relevant production module +
   its hermetic test:
   - PartitionedShardWriter: `src/barcada_scraper/output/parquet_writer.py`
     (the `write_to_dataset(filesystem=fs)` Hive path) +
     `tests/test_parquet_writer.py` +
     `tests/classifier/pipeline/test_parquet_writer_adls_azurite.py`
     (the S35 ShardWriter live test — closest precedent).
   - prompt log: `src/barcada_scraper/classifier/llm/prompt_logger.py`.
   - cost journal lease/SAS: `cost_journal_adls.py` + the S33/S34 tests.
   - page_storage (REFERENCE for the env-var-auth shape):
...
operator-led / E exhausted / S35+S36 candidates closed; NO
self-contained warm candidate remains; the named adlfs surfaces are
FRESH candidates). S37 may have no clearly-actionable engineering scope
without operator action. Then operator picks one.

### Three invocation shapes (S37 continues the empty-warm-queue condition)

- **Decided before invocation (cleaner).** Operator names the
  candidate when commissioning S37. Phase 1 CONFIRMS rather than
...
- **Decided at Phase 1 (more flexible).** S37 opens, Phase 0 verifies,
...
**Empirical prereq audit at S36 close** (carry-forward): 0 canary_runs
parquets; no barcada/canary/drift plist; no AI/ML responses. Re-run
these empirical checks at S37 Phase 1 BEFORE the candidate-choice
...
### NEW candidate space (remaining uncovered adlfs surfaces)

With the S35+S36 candidates closed, the remaining uncovered adlfs write
surfaces are explicit fresh candidates (each scoped at Phase 1/2 from
first principles, mirroring the S35/S36 live-test shape):

- **PartitionedShardWriter adlfs leg** — the Hive
  `write_to_dataset(filesystem=fs)` path; a DIFFERENT pyarrow code path
  than the S35 `ShardWriter`. Closes the second half of `parquet_writer.py`.
  A live test would mirror S35/S36 on port 10004.
- **prompt_logger adlfs leg** — fsspec `wb` single object (JSONL).
...
**survey coverage by CLIENT STACK, not by "is there an Azurite test"
(S35)**; default new external-service-backed tests to operator-smoke /
live-on-demand posture unless a named carve-out justifies CI (S30); if a
new live fixture must coexist with the S33/S34/S35/S36 fixtures, give it
a DISTINCT port + container name (10000/10001/10002/10003 taken → use
10004+); size new scripts with the ~70-100 LOC additive overhead floor.
...
- **Candidate D**: defer.
- **A NEW adlfs-surface candidate completing W A.2**: defer OR
  `workstream-a-week2-end` if the operator now considers W A.2 closed
  (OFFERED at S33, DEFERRED at S33 AND S34 AND S35 AND S36).
...
the `[[verify-before-asking-discipline]]` AND the S22-S36
...
W4.1.5 driver beyond the S16/S27/S28 exceptions OR to any S19-S36
...
adlfs test, or re-recording/deleting any of the 30 cassettes) — surface
as a design-gate sub-question before patching.

### CRITICAL Phase 2 hygiene

Carry forward S26/S28/S29/S30/S34/S35/S36 hygiene verbatim: count each
Q-* option set (≤4 or tier/split); audit existing test pins for
...
fold)**; and **read the production call-site SIGNATURE to determine the
auth seam before designing the auth option (S36 fold)**.
...
### If a NEW adlfs-surface candidate

Design the gates from first principles at session time, anchored on the
**S35/S36 live-test shape** (the closest precedents):
- Live marker, skip-by-default, Docker-gated, self-reproducing fixture
  ...
  baked `--skipApiVersionCheck`, DISTINCT port 10004+ + container name.
- **AUTH SEAM (S36): read the production call site's signature.** If it
  takes an explicit `storage_options=`/credential kwarg (like
  `ShardWriter`), pass the Azurite connection string through it. If it
  takes NONE (like `write_pages`), the auth is env-resolved — set
  `AZURE_STORAGE_CONNECTION_STRING` (via `monkeypatch.setenv`) and clear
  the adlfs instance cache before the production call. Verify which,
  do not assume.
- Container creation via `fs.mkdir` (catch `FileExistsError`), NOT
  `makedirs(..., exist_ok=True)` which silently no-ops (S35 fold).
...
- **Q-SHARED.1 Commit shape**: per-module / per-sub-surface (S18-S36
  default; Recommended) vs single bundled commit (only when genuinely
  one self-contained sub-surface, as the S35+S36 single live test files
  were).
...
before starting the next. Per S22-S36 precedent. If a mid-sub-surface
dependency emerges, HALT and surface as a design-gate sub-question.

### Build-time validation spike (carry the S35/S36 emphasis)
...
catches version-skew and API-semantic gotchas (e.g. S35's
`makedirs`-no-op; S36's `clear_instance_cache` need) inside Phase 3
instead of as a post-commit HALT. Tear the spike container down (it is
`--rm`; `docker stop` auto-removes — note that the agent's Bash `rm -f`
is env-hook-blocked, but `docker stop` and in-fixture `docker rm -f`
are fine). Clean up the spike script after (it must NOT be committed).
...
.venv/bin/python -m pytest \
    ...
    <new S37 test paths if any> -q
# A new @pytest.mark.live test stays OUT of the canonical headline
# unless a deliberate decision adds it (S33/S34/S35/S36 Option-1 posture).
# If placed under tests/classifier/pipeline/, it is auto-picked-up by the
# CI workflow's `-m live` selection but NOT by the canonical 16-path.
# Verify a new live test out-of-band:
...
# `ruff format` once — S35 reformatted; S36 reformatted once too. Apply
# it, re-check, re-run the test.)
```
...
LOC dropped 317→309 after format). **For any live test, include a teeth
check** (a negative control demonstrated to FAIL on the wrong path) in
the table, not just "it passes" (S34/S35/S36).
...
(`WA2.W8.*` is the established prefix for this surface family — S35/S36
operator-flagged this; verify against prior commits + SESSION_LOG before
committing). Commit message uses `-F` (no `-m` fallback) and contains
NO backticks in the body (S35 fold). NO `Co-Authored-By`.
...
S33/S34/S35/S36 pattern (`pytest -m live tests/classifier/pipeline/`).
...
edits to `eval_data/*` can fail the gate even though no S37 commit
...
2. Confirm the S37 commits do not touch eval_data
...
S20+S22 precedent: operator chose (a). S21/S23-S36 did not need this
...
the parquet adlfs leg (S35) + the page_storage adlfs leg (S36). The
`workstream-a-week2-end` tag was OFFERED at S33 and DEFERRED at S33,
S34, S35 AND S36; if S37 declares W A.2 closed it remains appropriate.

---

## Phase 6 — Workspace close-out

- Append Session 37 entry to `~/crawler-audit/SESSION_LOG.md` including
  a **Canonical S37-close baseline** block with the exact pytest
  invocation + verified count (per S22-S36 LESSONS). **If the fixture
  ...
  `exclusions_count == N` as the S38 Phase 0 Step 0.4 forward note.**
  Otherwise pin `cassette_count == 30` / `exclusions_count == 30`.
- Refill `~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md` for Session
  38 — explicitly pin the S38 Phase 0 workspace anchor SHA per the
  S21-S36 post-audit pattern. After the close-out commit lands, expect
...
Note: drafting the next-session prompt (`SESSION_38_PROMPT.md`) is NOT
a built-in Phase 6 step. Per S20→S36 precedent, prompt-drafting is
operator-commissioned between sessions. If the operator asks for it
explicitly at S37 close, draft it as a separate follow-up.
...
0. Phase 1 carve-out / justification recorded in chat (the surface is
   ...
   read from source (S36), and a demonstrated negative-control teeth
   check (S35)**.
...
- **S4.** Regression-protection checklist held (see Out-of-scope). In
  particular: ALL S21-S36 deliverables stay at their SHAs; their public
  ...
  10003, env-var auth, AzureBlobFileSystem teeth intact); per-sub-suite
  counts stay green; CRAWLING_POLICY.md 77 lines / 2519 bytes; the
  combined total stays at 970 (or grows) unless a Q-* authorizes a
...
Per the regression-protection checklist. **S36 added one new lock**
(the page_storage adlfs live test at `25c3696`); otherwise unchanged
from the S36 prompt.
...
- `tests/classifier/pipeline/test_parquet_writer_adls_azurite.py` at
  ...
  selection. **`output/parquet_writer.py` is EMPIRICALLY validated via
  the public `storage_options=` seam + `abfss://` URL; the module is
  UNMODIFIED — keep it that way.** `tests/test_parquet_writer.py` (33
  hermetic file:// tests) remains the default-run guard.

**S36 deliverables (NEW LOCK):**
- `tests/classifier/pipeline/test_page_storage_adls_azurite.py` at
  ...
  10003 / container `barcada-azurite-pages` (coexists with S33=10000 +
  S34=10001 + S35=10002 under one `-m live` run). **AUTH SEAM (distinct
  from S35): `write_pages` takes NO `storage_options` kwarg; the test
  authenticates adlfs via the `AZURE_STORAGE_CONNECTION_STRING` env var
  (`monkeypatch.setenv`) — production's real env-resolved path
  (`worker_loop.py:2305` uses the same two-arg signature).** Teeth:
  resolved fs is `AzureBlobFileSystem` + blob-relative path, read back
  via a FRESH adlfs handle; negative control = a `file://` fallback
  resolves to `LocalFileSystem` with an absolute path. Auto-joins the CI
  workflow's `-m live` dir selection. Do NOT modify without Phase 2
  authorization. **`page_storage.py` is EMPIRICALLY validated via the
  public `write_pages` seam + `abfss://` URL + env var; the module is
  UNMODIFIED — keep it that way.** `tests/classifier/page_acquisition/
...
expected and unstaged (Sessions 8-36); inter-session operator-side
...
S31+S32+S33+S34+S35+S36 added no new src/ authorizations (S35+S36 were
test only).
...
## Verify-before-asking discipline (strict rule from S19-S36)
...
wrong path (S35)**, and **that the auth seam claim matches the
production call-site signature (S36)**. No claims by pattern-completion.
...
- Before committing, confirm the subject's `WA2.W8.*` workstream label
  reconciles with the decision record.
...
- A live-emulator fixture must tear down on setup-phase failure
  ...
  live fixture needs a DISTINCT port + container name (S34/S35/S36:
  10000/10001/10002/10003 taken → 10004+).
- A build-time spike against the real emulator catches API-semantic
  gotchas inside Phase 3 instead of as a post-commit HALT (S35/S36).
...
- Plan-vs-reality at Phase 2 (S22); source-verify drives option-set
  design (S25; S28-S36).
- Test against public API surface only (S24; S29/S30/S33/S34/S35/S36).
...
- Markdown-rendered shell commands may carry NBSP / the env hook blocks
  ...
  the Write tool when exactness matters (S34/S35/S36).
- Operator-driven script LOC: ~70-100 LOC additive overhead floor (S29).
- Empirical Phase 1 prerequisite check before scope-narrow (S29; S30-S36
...
## Commit hygiene (per LESSONS + S19-S36 additions)
...
- Commit shape per Q-SHARED.1 (per-module default; single bundled commit
  ...
  as the S35+S36 single live test files were).
...
- Commit body includes: action ref, scope summary, file touches, test
  ...
  + the auth-seam source-verification (S34/S35/S36). NO `Co-Authored-By`.
...
- For S19-S36-test/fixture/workflow modifications: explicit Phase 2
...
- Workspace close-out (Phase 6) lands as its own commit, followed by
  1-2 follow-up commits pinning the anchor SHA for S38.

---

## Context-window awareness

S36 ran Phase 0 → Phase 6 in a single context window with 1 repo commit
(Phase 0 verify + Phase 1 scope + Phase 2 source-verify + operator
clarification on the auth seam + Phase 3 impl with a build-time spike +
close-out), comfortably within budget. S37 budget per scope:

- Candidate A: medium (~370-400 LOC); only if unblocked.
- Candidate D: small; operator-led.
- A NEW adlfs-surface candidate (PartitionedShardWriter / prompt_logger
  / lease-SAS): small-to-medium; size at Phase 2; mirror the S35/S36
  single-live-test shape.
...
- For any live-service-backed test, default to the S33/S34/S35/S36
  ...
  container name (10004+) if coexisting; container creation via `mkdir`
  not `makedirs(exist_ok=True)`; read the call-site signature for the
  auth seam (kwarg vs env); keep the test off the canonical headline
...
- If usage crosses ~70% before the chosen S37 scope closes, finish the
  in-flight sub-surface, then close session and refill the transition
  template for Session 38.

---

## Reporting in chat at session close

After all Session 37 commits land + push + close-out per the S13-36
pattern:

1. Commit SHA(s) of each S37 sub-surface.
2. Sub-surface(s) landed (per candidate).
3. Test count delta: 970 baseline → S37 close.
4. Driver suite count at S37 close (52/52 expected).
...
8. Live ADLS coverage status: K-b SHIP S29; K-b EXEC S30; K-a Azurite
   ...
   ShardWriter adlfs leg S35; page_storage adlfs leg S36. Report S37's
   disposition if touched.
9. Any spend (LLM, infrastructure).
10. Verify-before-asking summary: source-verification findings.
11. Outstanding items for Session 38.
12. Tags state at S37 close.
...
All S36 close-out commits (`cfb6851` primary + `fedb096` anchor-pin)
plus the S36 disposition have been folded directly into this prompt —
S37 does not need a separate amendment file:

- **S36 close-out anchors** folded into Step 0.1 (workspace anchor =
  ...
  public-API check).
- **1 NEW LESSONS section from S36** ("Survey the AUTH seam, not just
  the client stack") referenced in the Verify-before-asking LESSONS list.
- **S36 candidate CLOSED** (page_storage adlfs leg) folded into the
  carry-forward list + Scope section + Out-of-scope's S36 deliverable
  (NEW LOCK).
- **Candidate E still EXHAUSTED** (30 reached) — requires a
  plan-ceiling revision first.
- **NEW candidate space NAMED** — the remaining uncovered adlfs surfaces
  (PartitionedShardWriter Hive path / prompt_logger) + lease/SAS
  cost-journal paths, each a fresh Phase-2 candidate.

If new amendments arise pre-S37 open, walk them per the
