# Session 28 prompt — scope picked at Phase 1
# (S27 closed Candidate B + workstream-0-end tag; S28 chooses
#  from carry-forwards A/D/E/K + the NEW S27-deferred Stage 1
#  ShardResult split)

**Drafted at Session 27 close (2026-05-26).** Mirrors the
S20/S21/S22/S23/S24/S25/S26/S27 prompt structure. Scope-agnostic
at Phases 0/1; scope-specific design gates at Phase 2 per chosen
candidate. Strict 7-phase ordering with halt-on-mismatch
preserved.

This prompt should be invoked from `~/Downloads/session-28-prompt.md`
(operator-mirrored) or directly from
`~/crawler-audit/SESSION_28_PROMPT.md`. Re-read it on session open.
...
Session 27 closed Candidate B (1 commit + 2 close-out workspace
commits + 1 annotated tag). Repo HEAD at `a1c5636` with the
**11th tag `workstream-0-end`** placed at the closing commit;
Workstream 0 fully closes:

- **Candidate B shipped fully (S27)** —
  `tests/runners/fixture_cascade/cascade.py` rewired per-tier
  cost accounting; new `tests/runners/fixture_cascade/test_cost_journal_wiring.py`
  (338 LOC; 6 net-new tests). 6 of 8 `_TOTALS_FIELDS` slots
  populated from existing Stage 2/3 `ShardResult` components.
  Closed the S14 carry-forward "per-tier cost-accounting wiring
  gap". `workstream-0-end` tag placed at `a1c5636` with full
  W0-closure annotation. 964 → 970 combined-suite baseline.
...
- **barcada-drift (Candidate A)** — deferred per Q1.1=(A) at S20;
  ...
  date 2026-06-06 if the launchd installer fired ≥2 Saturdays
  since S20 close).

- **Phase 4 PR-D tooling (Candidate D)** — operator-led labeling
  territory. NOW UNBLOCKED on the W0 side (workstream-0-end
...
- **ADLSCostJournal live Azure smoke (Candidate K)** — carry-
  forward from S25 → S26 → S27 → S28. S25 shipped against
...
- **Stage 1 ShardResult LLM-vs-embedding split (NEW S27
  deferral)** — would close the remaining 2 of 8
  `_TOTALS_FIELDS` slots (`stage1_llm_usd`,
  `stage1_embedding_usd`) by exposing the LLM vs embedding cost
  split from `src/barcada_scraper/classifier/stage1/run.py` and
  `stage1/cost_tracker.py`. src/ scope — needs Phase 2 design-
  gate authorization for additive src/ touch. ~80-100 LOC.
  Tag implication: none (Workstream 0 already closed; this is a
  driver-area extension, not a new milestone).

Operator chooses at Phase 1 which candidate Session 28 ships.
Each candidate has its own Phase 2 design-gate template.

**Sessions 13-27 precedent:** this session does NOT modify the
W4.1.5 driver orchestration at `tests/runners/fixture_cascade/`
(locked at `dd64963` for original files; `cascade.py` extended at
`a1c5636` under S27 W5.X-prefix; `test_cost_journal_wiring.py`
added at `a1c5636` under same authorization). Does NOT modify
`expected.schema.json` v1.1 / `META_SCHEMA.md` v1.1. Does NOT
modify the committed `tests/fixtures/baseline-v0/` snapshot at
`9e9a1fb`. Does NOT modify the Session 19 `check` sub-surface
code or the Session 20 cassettes/canary sub-surface code or the
Session 21 `robots.py` parser at `34a59b6` or its tests, OR the
Session 22 deliverables (`scraper/robots_gate.py` @ `ba87e7e`,
`scraper/robots_bypass_config.py` @ `381ee89`, the
`cost_journal.py` S22 additions at `1d9404e`), OR the Session 23
...
`48c324a`), OR the Session 25 deliverables (`cost_journal_adls.py`
full backend at `835a531`, `test_cost_journal_adls.py` 19-test
expansion at `835a531`, the 3 1↔1 same-shape test replacements at
`835a531` + `aed7873`, the `_open_cost_journal_for_worker` body-
only abfss:// dispatch at `aed7873`), OR the Session 26 deliverable
(`docs/CRAWLING_POLICY.md` tightened at `2314f5e`; 77 lines /
2519 bytes), OR the Session 27 deliverables (the new
`_journal_record_with_breakdown` helper + Stage 2/3 invoker rewires
in `cascade.py` at `a1c5636`; `test_cost_journal_wiring.py` 6
tests at `a1c5636`; only the "Stage 1 ShardResult split" candidate
may extend the Stage 1 invoker in `cascade.py`, and only with
...
module (the Stage 1 ShardResult split candidate WILL need
explicit `src/stage1/run.py` + `src/stage1/cost_tracker.py`
authorization).
...
prompt before Session 28 starts, walk each flagged item against
on-disk reality at workspace HEAD `258cd86` and repo HEAD
`a1c5636` (or whatever HEAD the operator's machine carries),
BEFORE applying any change. Per S19-S27 pattern (LESSONS
...
halts catch hidden scope expansion (per S22-S27 "Implicit-
...
```
# Workspace at Session 28 start. S27 close-out landed across 2
# workspace commits: 5aab563 (primary close-out: SESSION_LOG.md
# + SESSION_TRANSITION_TEMPLATE.md + LESSONS.md) + 258cd86
# (anchor-pinning follow-up that succeeds 5aab563).
git -C ~/crawler-audit rev-parse HEAD
# Expect: 258cd86 (S27 anchor follow-up) OR a later commit if
# additional workspace doc edits or an S28 prompt draft landed
# post-S27-close. If N commits ahead, verify each prior commit's
# subject via `git log --oneline 258cd86..HEAD` against expected
# prompt-finalization / doc-edit patterns; surface the SHA delta
# and request authorization to proceed if anything is unexpected.
# (S20-S27 precedent: operator authorized continuation when 2-3
# extra workspace commits were the strengthened prompts
# themselves.)

# Repo at Session 27 final commit:
git -C /Users/administrator/projects/barcada-scraper rev-parse HEAD
# Expect: a1c5636 (WA0.W5.X.per-tier-cost-wiring) — the canonical
# end-of-S27 ship SHA.
#
# Tolerated delta: operator-side eval_data labeling commits between
# S27 close and S28 open are expected (Sessions 8-27 precedent).
...
# a1c5636..HEAD; surface any non-eval_data delta for operator
# authorization before continuing.
```

### Step 0.2 — Tags (NEW workstream-0-end at S27 close)

```
git -C /Users/administrator/projects/barcada-scraper tag -l | sort
# Expect 11 tags (10 → 11 since S26 close; NEW S27 = workstream-0-end):
#   baseline-v0
#   pre-remediation-2026-05-19
#   workstream-0-end                   ← NEW S27 at a1c5636
...
# Expect: a1c5636… (full SHA prefix matches Step 0.1 repo HEAD)
```

### Step 0.3 — Driver locked (with S16 + S27 exceptions)
...
### Step 0.4 — Fixture counts (no change from S27 close)
...
### Step 0.5 — Test-suite baseline (S28 canonical)
...
#
# Sub-totals (16 paths; driver grew 46 → 52 at S27;
#  all 15 other counts identical to S26 close):
...
#
# Pinned in SESSION_LOG.md "Canonical S27-close baseline" block.
# Re-verified post-S27-close-out at HEAD a1c5636: 970 passed in
# ~85s. The 970 count is invariant under operator-side eval_data
# commits between S27 and S28 (eval_data is not in the invocation).
```
...
**Narrower baselines** (valid for S28 candidates that don't
...
- 938 (S26 narrower 14-path; **retired post-S27** since the
  driver sub-suite grew 46→52 with the S27 per-tier-wiring tests,
  invalidating the count. Listed here as a historical reference
  so future debuggers landing on "why isn't 938 in the list?"
  can reconstruct the +6 delta. Use 944 instead.)
- 944 (S27-equivalent narrower for candidates that don't touch
  ADLS or robots_gate_integration; 16-path minus the 19
  cost_journal_adls + 7 robots_gate_integration; verified
  post-S27-close at a1c5636). Canonical 14-path invocation:
...
ALL Phase 3 commits in S28 — do not switch mid-session.
...
# S27 per-tier cost-accounting wiring tests (NEW)
...
### Step 0.9 — S25-shipped + S26-shipped + S27-shipped public API + doc stability (any-candidate prereq)

```
# Verify the S25 deliverables match what landed at S25 close, the
# S26 CRAWLING_POLICY.md tightening matches its landed shape, AND
# the S27 cascade.py per-tier wiring helper matches its landed
# signature. A change to any of these between S27 close and S28
# open would invalidate carry-forward candidate assumptions. Run
# unconditionally at every S28 cold start.
...
# Fail-loud-and-halt on any drift (per S26 close operator review).
...
# S27: invariant smoke — directly exercise the
# _journal_record_with_breakdown helper against an in-memory
# LocalFSCostJournal. NO corpus dependency (more robust against
# fixture rename / driver-kwarg drift across sessions than a
# cascade-driven smoke would be; same shape as
# test_journal_record_with_breakdown_chains_mutations in
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Exercise Stage 2 pattern (3 components, unattributed=0).
    ...
    # Exercise Stage 3 pattern (3 components, unattributed=fetch).
    _journal_record_with_breakdown(
        journal=journal,
        shard_id='s3-smoke',
        stage=3,
    ...
    # Use math.isclose for the cardinal invariant — exact float
    # equality would fail on accumulated decimal sums like
    # 0.10+0.20+0.30+0.05+0.07+0.03 (which evaluates to
    # 0.7500000000000002 rather than 0.75 exactly). 1e-9 abs_tol
    # is well within the sub-cent precision the journal needs.
    assert math.isclose(
        state.totals.cost_usd,
        per_tier_sum + shard_sum,
        abs_tol=1e-9,
    ), (state.totals.cost_usd, per_tier_sum, shard_sum)
    # 8 per-tier slots present on CostTotals.
    for fname in cj._TOTALS_FIELDS.values():
        assert hasattr(state.totals, fname), fname

print('OK S27 per-tier wiring invariant holds at S28 cold start (direct-API smoke; no corpus dependency)')
"
```
...
## Required workspace reading (Session 28 first 10 minutes)

In this order:

1. **`~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md`** — full
   handoff state at S27 close. Lists scope candidates (A/D/E/K +
   NEW Stage 1 ShardResult split) with prerequisites + estimated
   scope. The S28 scope choice at Phase 1 picks from these.

2. **`~/crawler-audit/SESSION_LOG.md`** Session 27 entry — what
   landed during Candidate B (1 commit + 2 close-out workspace
   commits + 1 annotated tag). The S27 disposition paragraph
   explicitly documents which `_TOTALS_FIELDS` slots stay $0 and
   why; the workstream-0-end tag annotation covers all of W0.

3. **`~/crawler-audit/LESSONS.md`** — 1 new section folded at
   S27 close, at end of file. Locate via
   `grep -n '^## .*(S27 folding)' LESSONS.md`. Read with care:
   - "Deferred wiring gaps fold cleanly into workstream-end if
     the original implementation left a parallel-API seam" —
     covers the S14 → S27 deferred-to-closed cycle. Includes
     forward-applicable detection protocols at BOTH deferral
...
6. **`~/crawler-audit/PIPELINE_ARCHITECTURE_TARGET_STATE.md`** —
   only if Stage 1 ShardResult split candidate touches the
   stage1/run.py invariants. READ-ONLY.

7. **`src/barcada_scraper/classifier/pipeline/cost_journal_adls.py`**
   — only if Candidate K touches the ADLS backend. 295 LOC.
   The full Phase 5 backend shipped at S25 SHA `835a531`.

8. **`tests/classifier/pipeline/test_cost_journal_adls.py`** at
   `835a531` — only if Candidate K touches ADLS testing. 19
...
10. **`tests/runners/fixture_cascade/cascade.py`** at S27 SHA
    `a1c5636` — only if Stage 1 ShardResult split candidate is
    chosen. The module docstring explicitly documents which
    `_TOTALS_FIELDS` slots stay $0 and why; the Stage 1 invoker
    block is the extension point.

11. **`src/barcada_scraper/classifier/stage1/run.py`** + the
    `ShardResult` dataclass at session-current HEAD — only if
    Stage 1 ShardResult split candidate is chosen. Source-verify
    line numbers at session-current HEAD before drafting commit
    edits; the file may have shifted since the Phase 0.9 API
    snapshot above was taken.

12. **`src/barcada_scraper/classifier/stage1/cost_tracker.py`**
    at session-current HEAD — only if Stage 1 ShardResult split
    candidate is chosen. The cost tracker is where LLM vs
    embedding costs accrue separately if at all; verify the
    accrual structure before drafting Q-StgSplit options.
...
Session 28 AND operator has not authorized explicit placeholder
choices.
...
tooling. **NOW UNBLOCKED on the W0 side** (workstream-0-end tag
at `a1c5636` closed W0); still gated on operator-led labeling.
...
goal; K-a if a permanent CI safety net is desired.

### Candidate StgSplit — Stage 1 ShardResult LLM-vs-embedding split (NEW S27 deferral)

Closes the remaining 2 of 8 `_TOTALS_FIELDS` slots
(`stage1_llm_usd`, `stage1_embedding_usd`) that the S27 retrofit
left at $0. Requires src/ scope — touches:
- `src/barcada_scraper/classifier/stage1/run.py` (ShardResult
  dataclass + the run_shard cost accrual path).
- `src/barcada_scraper/classifier/stage1/cost_tracker.py` (LLM
  vs embedding cost accrual; current shape needs verification).
- `tests/runners/fixture_cascade/cascade.py` (extend the Stage 1
  invoker to use `_journal_record_with_breakdown` with
  components={'llm': ..., 'embedding': ...} once the ShardResult
  exposes both fields).
- `tests/runners/fixture_cascade/test_cost_journal_wiring.py`
  (update `test_stage1_per_tier_slots_remain_zero_by_design` to
  the NEW expected shape, or 1↔1 replace with
  `test_stage1_per_tier_slots_populate_from_split`).
- Stage 1 ShardResult-level tests in
  `tests/classifier/stage1/` (new tests pinning the split).

Estimated ~80-100 LOC src/ + ~30 LOC test updates + Phase 2
explicit src/ authorization required.

**Prerequisites:**
- Operator authorization at S28 Phase 1 for additive src/ touch
  on `src/barcada_scraper/classifier/stage1/run.py` and
  `stage1/cost_tracker.py`. Without this auth, the candidate
  HALTs at Phase 1.
- Decision on whether the Stage 1 LLM cost field is named
  `stage1_llm_cost_usd` (parallel to Stage 2/3) or some other
  identifier — source-verification of the existing CostTracker
  fields at session-current HEAD WILL drive this decision per
  S25-folded "Phase 2 source-verify drives option-set design".
...
- **Candidate StgSplit** (Stage 1 split): defer (no workstream
  milestone left — W0 already closed at S27).
...
before-asking-discipline]]` AND per S22-S27 "Plan-vs-reality at
...
authorized (per S22-S27 "Implicit-authorization HALT for src/-
locks" LESSONS pattern; surface as an explicit AskUserQuestion
before patching) OR to the W4.1.5 driver beyond the S16/S27
W5.X-prefix exceptions OR to any S19-S27 deliverable — surface as
...
### **CRITICAL Phase 2 hygiene from S27 LESSONS** (apply when deferring or closing carry-forward gaps)

Per S27-folded LESSONS "Deferred wiring gaps fold cleanly into
workstream-end if the original implementation left a parallel-API
seam": when scoping a candidate that closes a deferred gap,
re-source-verify at Phase 2 that the seam (parallel-API method,
existing extension point, ShardResult field) still exists and
still has the docstring/signature the deferral predicted. APIs
sometimes drift between sessions; treating a years-old deferral
disposition as gospel without re-verifying is the path to
wrong-Q-* options.

### If Candidate A (barcada-drift)

Carry-forward from S22-S27 prompts (unchanged).
...
### If Candidate StgSplit (Stage 1 ShardResult LLM-vs-embedding split)

Apply the **CRITICAL Phase 2 hygiene from S27 LESSONS** block
above before drafting Q-StgSplit.* options (the seam-verification
discipline; do not restate it here). Specifically source-verify:
- Does `stage1/cost_tracker.py` already track LLM vs embedding
  costs separately, or only an aggregate? Determines whether the
  split is a *plumbing* change (expose existing data) or a
  *tracking* change (begin separating accumulation).
- Does `stage1_run.ShardResult` have natural extension points
  (new dataclass fields) or does the call site assert on the
  current field count?
- How do existing Stage 1 tests in `tests/classifier/stage1/`
  pin the cost shape — by field count, by aggregate value, or
  by component?

Then draft (recommended Q-* set; tier or split if any Q-*
exceeds 4 options):
- **Q-StgSplit.1 Cost-tracker semantic**: expose pre-existing
  separate accumulation vs introduce new separate accumulation
  vs aggregate-only-with-attribution-flag.
- **Q-StgSplit.2 ShardResult shape**: add 2 new fields
  (`llm_cost_usd`, `embedding_cost_usd`) keeping `cost_usd` as
  aggregate (Recommended; backwards-compatible) vs add a nested
  `cost_breakdown` substructure (also backwards-compatible) vs
  replace `cost_usd` with the split (**Not recommended;
  pre-S25-style API break** — breaks `cascade.py` Stage 1
  invoker's `s1_result.cost_usd` consumer at the
  `_journal_record(..., cost_usd=s1_cost, ...)` call site,
  the `CascadeResult.stage1_cost_usd` aggregate field, and
  any downstream consumer pinning the current field set).
- **Q-StgSplit.3 cascade.py invoker update**: extend Stage 1
  block to use `_journal_record_with_breakdown` with
  components={'llm': ..., 'embedding': ...} vs leave Stage 1
  invoker using the existing `_journal_record` helper and bump
  per-tier slots in a separate call.
- **Q-StgSplit.4 test_cost_journal_wiring.py update**:
  1↔1 replace `test_stage1_per_tier_slots_remain_zero_by_design`
  with `test_stage1_per_tier_slots_populate_from_split` (Q-J.8-
  style same-shape replacement; net-zero test count) vs add a
  new test alongside (net +1) vs leave the existing test and
  add coverage at the stage1/ unit-test level.
- **Q-StgSplit.5 Backfill behavior**: same as Q-B.2 (no backfill
  recommended) vs regenerate baseline-v0 vs document gap.
- **Q-StgSplit.6 Commit shape**: per-module commits (S18-S27
  default) — likely 3 commits (cost_tracker change → run.py
  change → cascade.py/test wiring update) vs bundled per-sub-
  surface.
- **Q-StgSplit.7 src/ authorization scope**: stage1/run.py +
  stage1/cost_tracker.py only vs broader stage1/ package vs
  any other src/ surface implied by the cost-tracker change.

(NB: Q-StgSplit has 7 gates — more than any prior candidate.
Two AskUserQuestion caps apply independently and BOTH need to
be respected:
- 4 OPTIONS per question (the S26 LESSONS topic): check each
  Q-StgSplit.* individually; tier/split via the CRITICAL
  hygiene block above if any single Q-StgSplit.* exceeds 4
  mutually-exclusive options. (Note: Q-StgSplit.2 as worded
  has 3 options; Q-StgSplit.1/3/4/5/6/7 each have 2-3; no
  Q-* currently exceeds 4 but source-verification at S28 open
  may surface a 4th option that does.)
- 4 QUESTIONS per call (a separate tool-schema cap): batching
  all 7 Q-StgSplit into one AskUserQuestion would violate this;
  split into 2-3 sequential calls (e.g., 3+2+2, 4+3, or 3+4).
  Pair related Q-* in the same call (e.g., Q-StgSplit.1 +
  Q-StgSplit.2 are both about cost-data shape and read well
  together).)

### Shared sub-questions (all candidates)

- **Q-SHARED.1 Commit shape**: per-module (S18-S27 default;
...
If the chosen candidate has multiple sub-surfaces (Candidate
StgSplit likely does: cost-tracker change + run.py ShardResult
change + cascade.py invoker wiring + test updates), do NOT
interleave. Complete each sub-surface fully before starting the
next. Per S22-S27 precedent.
...
the format-fix into the commit (per S19-S27 pattern).
...
sessions (Sessions 8-27 precedent).
...
```
Phase 3 start                  : 970  (Session 27 close baseline)
...
canonical S28 baseline for any candidate that touches the
orchestrator sub-surface OR the driver subsurface OR the journal
sub-surface (Candidates K + StgSplit definitely; A/D/E less so).
Whichever baseline is bound at Phase 1, hold it consistent across
ALL Phase 3 commits in S28 — do not switch mid-session.
...
S20+S22 precedent: operator chose (a). S21/S23/S24/S25/S26/S27 did
...
No imminent workstream-A.2-end tag is anticipated for S28 unless
operator scopes it explicitly.

---

## Phase 6 — Workspace close-out

- Append Session 28 entry to `~/crawler-audit/SESSION_LOG.md`
  including a **Canonical S28-close baseline** block with the
  exact pytest invocation + verified test count (per S22-S27
  LESSONS "Pin the baseline for the next session's Phase 0
  Step 0.5"; without this, S29 Phase 0 will infer from the
  "Combined headline" bullet and potentially HALT on
  accounting-mismatch).
- Refill `~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md` for
  Session 29 — explicitly pin the S29 Phase 0 workspace anchor
  SHA per S21-S27 post-audit pattern (LESSONS "Workspace HEAD
  delta tolerance"); do not omit. After the close-out commit
  lands, expect **1-2 follow-up commits** pinning the actual
  SHA (S21 needed 1; S22 needed 2; S27 needed 1).
...
Note: drafting the next-session prompt (`SESSION_29_PROMPT.md`)
is NOT a built-in Phase 6 step. Per S20→S21..S26→S27→S28
precedent, prompt-drafting is operator-commissioned between
sessions — not always-on close-out work. If the operator asks for
it explicitly at S28 close, draft it as a separate follow-up;
otherwise leave for the next session to either operator-commission
or scope out at S29 open.
...
### Candidate StgSplit (Stage 1 ShardResult split)

1. `_TOTALS_FIELDS` slots `(1, 'llm')` and `(1, 'embedding')`
   populate from real Stage 1 cost flow after the cascade runs
   (no longer $0).
2. The cardinal invariant
       totals.cost_usd == sum(shard.cost_usd) + sum(per-tier
                                                  slot values)
   continues to hold across the full cascade run (S27 cardinal
   test in `test_cost_journal_wiring.py` stays green or is 1↔1
   replaced per Q-StgSplit.4).
3. Stage 1 ShardResult changes are additive (existing aggregate
   `cost_usd` field stays available unless Q-StgSplit.2 chose
   "replace").
4. Stage 1 unit tests at `tests/classifier/stage1/` updated to
   pin the new LLM-vs-embedding split (or new tests added per
   Q-StgSplit.4).

**Post-implementation observations (not gating):**
- The `workstream-0-end` tag annotation at `a1c5636` was written
  assuming the Stage 1 ShardResult split would be deferred
  indefinitely. If Candidate StgSplit lands at S28, the
  annotation is technically over-pessimistic on Stage 1 wiring
  status. Rewriting the annotation requires deleting and
  re-placing the tag (hard-to-reverse on a published tag);
  leave in place unless the operator explicitly requests a
  retag. Document the supersession in SESSION_LOG.md instead —
...
- **S4.** Regression-protection checklist held (see "Out-of-scope"
  below). In particular:
  - ALL S21-S27 deliverables stay at the SHAs they landed at;
    their public APIs are unchanged (with the explicit exception
    of stage1/run.py + cost_tracker.py if Candidate StgSplit is
    chosen and Q-StgSplit.7 authorizes the touch).
  ...
  - `tests/runners/fixture_cascade/cascade.py`'s
    ...
    from S27 `a1c5636` (verified at Phase 0 Step 0.9); the
    Stage 1 invoker block may extend ONLY if Candidate StgSplit
    is chosen and Q-StgSplit.3 authorizes the extension.
...
- `src/barcada_scraper/classifier/pipeline/cost_journal_adls.py`
  ...
  Candidate K may CONSUME `ADLSCostJournal` but MUST NOT modify
  the public API signatures without explicit Phase 2
...
- `docs/CRAWLING_POLICY.md` at S26 SHA `2314f5e` (77 lines /
  ...
  8.1 KB to 2.52 KB). No candidate at S28 should modify this doc
  unless Phase 2 design-gate explicitly authorizes further
  tightening, extension, or rewrite.

**S27 deliverables (Session 27 W A.0 W5.X sub-surface — NEW for S28 lock-list):**
- `tests/runners/fixture_cascade/cascade.py` per-tier wiring
  changes at S27 SHA `a1c5636`:
    - Module docstring "Per-tier cost-accounting wiring
      (WA0.W5.X, S27 Candidate B)" paragraph.
    ...
    - Stage 1 invoker continues to use the existing
      `_journal_record` helper (aggregate cost_usd only). This
      is the EXTENSION POINT for Candidate StgSplit only; any
      other candidate MUST leave the Stage 1 invoker unchanged.
- `tests/runners/fixture_cascade/test_cost_journal_wiring.py` at
  S27 SHA `a1c5636` (338 LOC; 6 tests). Candidate StgSplit may
  1↔1 REPLACE `test_stage1_per_tier_slots_remain_zero_by_design`
  per Q-StgSplit.4 (net-zero count); other candidates MUST NOT
  modify this file.

**W4.1.5 driver orchestration (locked since W4.1.5 close):**
- `tests/runners/fixture_cascade/` (except via W5.X-prefix commit
  with explicit operator auth — only Candidates B and StgSplit
  open this surface; B is closed at S27 via `a1c5636`).
...
- All workstream tags at their placed SHAs (11 tags as of S27
...
- All of `eval_data/` — labeling-workstream territory; per-row WIP
  edits across sessions are expected and unstaged (Sessions 8-27
  precedent). S22→S27 inter-session operator-side eval_data
...
- `src/barcada_scraper/` — locked unless Phase 2 design-gate
  ...
  S27 added no new src/ authorizations (driver-only W5.X-prefix).
  Candidate StgSplit at S28 WILL need explicit additive src/
  authorization for `classifier/stage1/run.py` +
  `classifier/stage1/cost_tracker.py` per Q-StgSplit.7.
  Those authorizations do NOT extend to other src/ modules or
  to further modifications of the authorized files beyond their
  landed S21-S27 shapes.

**Pipeline configs:**
- `configs/`

**Phase 4 work:**
- Phase 4 PR-D/E/F/G work UNBLOCKED on the W0 side as of S27
  (workstream-0-end placed); still gated on operator-led Stage 2/3
  labeling. Candidate D opens when labeling begins.

---

## Verify-before-asking discipline (strict rule from S19-S27)
...
- Before each chosen-candidate-specific claim in a commit message,
  ...
  - Candidate StgSplit: source-verify Stage 1 cost-tracker
    behavior BEFORE drafting Q-StgSplit.1; the cost-tracker
    shape predicts whether the split is "expose existing data"
    or "begin separating accumulation". Then verify per-tier
    slot values in the post-cascade journal match the expected
    LLM-vs-embedding split (e.g., run a fake-mode cascade with
    injected non-zero `cost_per_call_usd` and confirm
    `stage1_llm_usd > 0` and `stage1_embedding_usd >= 0`
    populate).
...
LESSONS-folded discoveries from S22-S27 worth re-applying:
...
- **Implicit-authorization HALT** (S22-S27 LESSONS): Q-*
  answers may require touching files NOT in S28 Out-of-scope's
  authorized-touch list (e.g., introducing a new helper module
  for ADLS-specific live smoke; extending stage1/cost_tracker.py
  for the LLM-vs-embedding split). Surface explicit authorization
...
- **Test against public API surface only** (S24 LESSONS): tests
  should probe behavior via the public surface. Avoid asserting
  on private attrs.

- **Source-verify line numbers per Phase 3 commit** (S23
  LESSONS): worker_loop.py is now ~3000+ LOC post-S25; stage1/
...
## Commit hygiene (per LESSONS + S19-S27 additions)
...
- Commit body includes: action ref (e.g., `WA0.W5.X.stage1-cost-split`
  for Candidate StgSplit), scope summary, file touches, test count
...
- For S24-S27-test modifications: explicit Phase 2 authorization
  required; document the modification scope explicitly in the
  commit body (per S22-S27 LESSONS "Implicit-authorization HALT"
  pattern).
- Workspace close-out (Phase 6) lands as its own commit at session
  ...
  for the next session (S21-S27 pattern). S26 demonstrated that a
  POST-close-out correction commit is sometimes necessary; when
  this happens, the new HEAD supersedes the anchor-pinning follow-up,
  and the next session's Phase 0 will need to tolerate the
  additional commit per the Workspace HEAD delta tolerance pattern.

---

## Context-window awareness

S27 ran across 1 commit + Phase 2 source-verification + Phase 6
close-out, well within context. S28 budget per scope:
...
- Candidate StgSplit: medium (~80-100 LOC src/ + ~30 LOC test
  updates); 3-4 commits likely under per-module shape.

Strategies:
- Use Edit over Write for small additions.
- Delegate multi-file design-of-record analysis to Explore
  subagent per S22-S27 "Explore-subagent + spot-check" LESSONS
  pattern.

Self-monitor cadence:
- Report estimated context usage at ~30%, ~60%.
- If usage crosses ~70% before the chosen S28 scope closes,
  transition per "no mid-commit-batch transitions" — finish
  in-flight sub-surface, then close session and refill the
  transition template for Session 29.

---

## Reporting in chat at session close

After all Session 28 commits land + push + close-out per the
S13-27 pattern:

1. Commit SHA(s) of each S28 sub-surface.
2. Sub-surface(s) landed (per candidate).
3. Test count delta: 970 (or 480/538/944) baseline → S28 close.
4. Driver suite count at S28 close (52/52 expected unless
   Candidate StgSplit added new driver-area tests).
5. Files touched per sub-surface.
6. Tag dispositions (incl. any new tag per 1.TAG).
7. Per-tier cost-accounting wiring gap disposition: closed-fully
   if Candidate StgSplit landed (all 8 _TOTALS_FIELDS slots
   populate) OR remaining 2-slot residual if not.
...
12. Verify-before-asking summary: any source-verification
    findings surfaced.
13. Outstanding items for Session 29.
14. Tags state at S28 close.
...
All S27 close-out commits (`5aab563` primary + `258cd86` anchor
follow-up) plus the S27 deliverables have been folded directly
into this prompt — S28 does not need a separate amendment file:

- **S27 close-out anchors** folded into Step 0.1 (workspace
  anchor `258cd86`; repo anchor `a1c5636` with operator-eval_data-
  commit tolerance), Step 0.5 (Canonical S27-close baseline 970
  pinned with 16-path invocation + sub-totals; 944 narrower for
  candidates that don't touch ADLS or robots_gate_integration),
  Step 0.9 (S24+S25+S26 stability PLUS new S27
  `_journal_record_with_breakdown` signature check + per-tier
  invariant smoke).
- **1 LESSONS section from S27 close** ("Deferred wiring gaps
  fold cleanly into workstream-end if the original implementation
  left a parallel-API seam") referenced where it applies: at
  Phase 2 "CRITICAL Phase 2 hygiene from S27 LESSONS" (apply
  when deferring or closing carry-forward gaps) AND at the
  Verify-before-asking section's LESSONS-folded-discoveries list.

If new amendments arise pre-S28 open, walk them per the reviewer-
