# Session 29 prompt — scope picked at Phase 1
# (S28 closed Candidate StgSplit + post-close hygiene fold;
#  S29 chooses from carry-forwards A/D/E/K)

**Drafted at Session 28 close (2026-05-26).** Mirrors the
S20/S21/S22/S23/S24/S25/S26/S27/S28 prompt structure. Scope-agnostic
at Phases 0/1; scope-specific design gates at Phase 2 per chosen
candidate. Strict 7-phase ordering with halt-on-mismatch
preserved.

This prompt should be invoked from `~/Downloads/session-29-prompt.md`
(operator-mirrored) or directly from
`~/crawler-audit/SESSION_29_PROMPT.md`. Re-read it on session open.
...
Session 28 closed Candidate StgSplit (3 commits + 2 workspace
close-out commits + 1 post-close hygiene commit). Repo HEAD at
`ae9e627`; workspace HEAD at `3089faa`. **Workstream 0 fully
closed at S27 via the `workstream-0-end` tag at `a1c5636`**; S28
shipped the carry-forward Stage 1 ShardResult LLM-vs-embedding
split that S27 deferred. No new W0-tag implicated for S29.

Carry-forward candidates entering S29 (StgSplit fully closed S28):
...
- **ADLSCostJournal live Azure smoke (Candidate K)** — carry-
  forward from S25 → S26 → S27 → S28 → S29. S25 shipped
  ADLSCostJournal against `DummyBlobBackend` in-memory (Q-J.3 (c));
  no production code path has been exercised against real Azure.
  A small operator-driven smoke test (write_initial + try_update
  + read against a sandbox container) would close the mock-vs-
  prod divergence risk.

**No new carry-forward introduced by S28.** The Stage 1
ShardResult split closed cleanly end-to-end; the per-tier cost-
accounting wiring gap is closed for all 8 `_TOTALS_FIELDS` slots.

Operator chooses at Phase 1 which candidate Session 29 ships.
Each candidate has its own Phase 2 design-gate template.

**Sessions 13-28 precedent:** this session does NOT modify the
...
module. None of the S29 candidates (A/D/E/K) currently anticipate
new src/ authorizations beyond what's already authorized for
...
prompt before Session 29 starts, walk each flagged item against
on-disk reality at workspace HEAD `3089faa` and repo HEAD
`ae9e627` (or whatever HEAD the operator's machine carries),
BEFORE applying any change. Per S19-S28 pattern (LESSONS
...
halts catch hidden scope expansion (per S22-S28 "Implicit-
...
```
# Workspace at Session 29 start. S28 close-out landed across 3
# workspace commits: 2bb5879 (primary close-out: SESSION_LOG.md
# + SESSION_TRANSITION_TEMPLATE.md + LESSONS.md) + 85a84c4
# (anchor-pinning follow-up) + 3089faa (post-close hygiene fold
# for Phase 0 fixture-count commands + S29 prompt-drafting pointer).
git -C ~/crawler-audit rev-parse HEAD
# Expect: 3089faa (S28 post-close hygiene fold) OR a later commit if
# additional workspace doc edits or an S29 prompt draft landed
# post-S28-close. If N commits ahead, verify each prior commit's
# subject via `git log --oneline 3089faa..HEAD` against expected
...
# (S20-S28 precedent: operator authorized continuation when 1-2
# extra workspace commits were the strengthened prompts
# themselves.)

# Repo at Session 28 final commit:
git -C /Users/administrator/projects/barcada-scraper rev-parse HEAD
# Expect: ae9e627 (WA0.W5.X.stage1-cost-split-wiring-test) — the
# canonical end-of-S28 ship SHA.
#
# Tolerated delta: operator-side eval_data labeling commits between
# S28 close and S29 open are expected (Sessions 8-28 precedent).
...
# ae9e627..HEAD; surface any non-eval_data delta for operator
# authorization before continuing.
```

### Step 0.2 — Tags (unchanged from S27 close)

```
git -C /Users/administrator/projects/barcada-scraper tag -l | sort
# Expect 11 tags (UNCHANGED since S27 close; S28 placed no tag):
#   baseline-v0
...
### Step 0.5 — Test-suite baseline (S29 canonical)
...
#
# Sub-totals (16 paths; ALL identical to S27 close — S28's 1↔1
# same-shape replacement inside the 52 driver count preserved
# net total):
...
# commits between S28 and S29 (eval_data is not in the invocation).
```
...
**Narrower baselines** (valid for S29 candidates that don't
...
- 944 (S27/S28-equivalent narrower for candidates that don't touch
  ADLS or robots_gate_integration; canonical 16-path minus 19
  cost_journal_adls minus 7 robots_gate_integration; verified
  post-S28-close at ae9e627). Canonical 14-path invocation:
...
ALL Phase 3 commits in S29 — do not switch mid-session.
...
# signature, AND the S28 Stage 1 ShardResult split + cascade.py
# Stage 1 invoker shape match their landed structure. A change to
# any of these between S28 close and S29 open would invalidate
# carry-forward candidate assumptions. Run unconditionally at
# every S29 cold start.
...
# LocalFSCostJournal. NO corpus dependency.
.venv/bin/python -c "
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Exercise Stage 1 pattern (S28 NEW: 2 components, unattributed=0).
    _journal_record_with_breakdown(
        ...
        components={'llm': 0.04, 'embedding': 0.01},
    ...
    _journal_record_with_breakdown(
        ...
        shard_id='s3-smoke',
        stage=3,
        started_at='2026-05-26T00:02:00+00:00',
        domains_processed=10,
    ...
    # All 8 _TOTALS_FIELDS slots populate after S28 closure.
...
print('OK S27+S28 per-tier wiring invariant holds at S29 cold start (all 8 slots populate)')
"

# S28 NEW: ShardResult split field presence (compile-time guarantee).
.venv/bin/python -c "
...
# 14 fields total: 9 n_* + cost_usd + llm_cost_usd + embedding_cost_usd
# + cached_input_tokens + output_path.
...
# call with stage=1 + 'llm' + 'embedding' components.
.venv/bin/python -c "
import ast
with open('tests/runners/fixture_cascade/cascade.py') as f:
    tree = ast.parse(f.read())

# Find all _journal_record_with_breakdown calls.
all_calls = []
stage1_calls = []
...
# Expect 3 total calls (one per stage), with exactly 1 at stage=1.
...
# Verify the Stage 1 call has llm + embedding components.
call = stage1_calls[0]
...
## Required workspace reading (Session 29 first 10 minutes)

In this order:

1. **`~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md`** — full
   handoff state at S28 close. Lists scope candidates (A/D/E/K)
   with prerequisites + estimated scope. The S29 scope choice
   at Phase 1 picks from these.

2. **`~/crawler-audit/SESSION_LOG.md`** Session 28 entry — what
   landed during Candidate StgSplit (3 commits + 2 close-out
   workspace commits + 1 post-close hygiene commit). The S28
   disposition paragraph documents all 8 `_TOTALS_FIELDS` slots
   are now wired and the "by design vs empirical" test-pin
   distinction that surfaced.

3. **`~/crawler-audit/LESSONS.md`** — 2 new sections folded at
   S28 close, at end of file. Locate via
   `grep -n '^## .*(S28 folding\|S28 post-close folding)' LESSONS.md`.
   Read with care:
   - "Empirical-vs-by-design distinction in test pins" (S28
     folding) — covers test-assertion semantic drift when a
     deferred gap closes via a parallel-API seam.
   - "Phase 0 fixture-count commands need `2>/dev/null` + a
     bounded timeout" (S28 post-close folding) — **MANDATORY
     READ for any Phase 0 modification**. Documents why bare
     `find` invocations hung in S28 and the Python pattern that
     replaces them.
...
6. **`~/crawler-audit/PIPELINE_ARCHITECTURE_TARGET_STATE.md`** —
   only if Candidate K touches ADLS Phase 5 promotion
...
10. **`tests/runners/fixture_cascade/cascade.py`** at S28 SHA
    `9afde57` — for reference if any candidate touches the
    driver. Module docstring documents that all 8 `_TOTALS_FIELDS`
    slots are now wired (Stage 1 closure shipped S28).

11. **`src/barcada_scraper/classifier/stage1/run.py`** at S28 SHA
...
Session 29 AND operator has not authorized explicit placeholder
choices.
...
before-asking-discipline]]` AND per S22-S28 "Plan-vs-reality at
...
authorized (per S22-S28 "Implicit-authorization HALT for src/-
...
W5.X-prefix exceptions OR to any S19-S28 deliverable — surface as
...
Carry-forward from S22-S28 prompts (unchanged).
...
- **Q-SHARED.1 Commit shape**: per-module (S18-S28 default;
...
the format-fix into the commit (per S19-S28 pattern).
...
sessions (Sessions 8-28 precedent).
...
```
Phase 3 start                  : 970  (Session 28 close baseline)
...
canonical S29 baseline for any candidate that touches the
orchestrator sub-surface OR the driver subsurface OR the journal
sub-surface (Candidate K definitely; A/D/E less so).
Whichever baseline is bound at Phase 1, hold it consistent across
ALL Phase 3 commits in S29 — do not switch mid-session.
...
No imminent workstream-A.2-end tag is anticipated for S29 unless
...
- Append Session 29 entry to `~/crawler-audit/SESSION_LOG.md`
  including a **Canonical S29-close baseline** block with the
  exact pytest invocation + verified test count (per S22-S28
  LESSONS "Pin the baseline for the next session's Phase 0
  Step 0.5"; without this, S30 Phase 0 will infer from the
  "Combined headline" bullet and potentially HALT on
  accounting-mismatch).
- Refill `~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md` for
  Session 30 — explicitly pin the S30 Phase 0 workspace anchor
  SHA per S21-S28 post-audit pattern (LESSONS "Workspace HEAD
  ...
  including a post-close hygiene fold).
...
Note: drafting the next-session prompt (`SESSION_30_PROMPT.md`)
...
it explicitly at S29 close, draft it as a separate follow-up;
otherwise leave for the next session to either operator-commission
or scope out at S30 open.
...
- **S4.** Regression-protection checklist held (see "Out-of-scope"
  below). In particular:
  - ALL S21-S28 deliverables stay at the SHAs they landed at;
...
- `docs/CRAWLING_POLICY.md` at S26 SHA `2314f5e` (77 lines /
  ...
  8.1 KB to 2.52 KB). No candidate at S29 should modify this doc
...
- `tests/runners/fixture_cascade/test_cost_journal_wiring.py` at
  ...
  to reflect Stage 1 closure. No candidate at S29 should
  modify this file unless Phase 2 design-gate explicitly
  authorizes.

**S28 deliverables (Session 28 carry-forward closure — NEW for S29 lock-list):**
...
- All of `eval_data/` — labeling-workstream territory; per-row WIP
  edits across sessions are expected and unstaged (Sessions 8-28
  precedent). S22→S28 inter-session operator-side eval_data
...
- `src/barcada_scraper/` — locked unless Phase 2 design-gate
  ...
  S26 + S27 added no new src/ authorizations (doc-only or
  driver-only).
  None of the S29 candidates currently anticipate new src/
  authorizations.
...
## Verify-before-asking discipline (strict rule from S19-S28)
...
LESSONS-folded discoveries from S22-S28 worth re-applying:
...
- **Phase 2 source-verify drives option-set design** (S25 LESSONS):
  ...
  src/ scope estimate from ~80-100 LOC to ~10-15 LOC.

- **Implicit-authorization HALT** (S22-S28 LESSONS): Q-*
  answers may require touching files NOT in S29 Out-of-scope's
...
## Commit hygiene (per LESSONS + S19-S28 additions)
...
- Commit body includes: action ref (e.g., `WA0.W5.X.stage1-cost-split`
  was S28's; S29 chooses its own per candidate), scope summary,
  file touches, test count delta (with net-new vs newly-in-
  invocation pre-existing distinction per S23 LESSONS), plan
  reference. NO `Co-Authored-By`.
...
- For S19-S28-test modifications: explicit Phase 2 authorization
  required; document the modification scope explicitly in the
  commit body (per S22-S28 LESSONS "Implicit-authorization HALT"
  pattern).
- Workspace close-out (Phase 6) lands as its own commit at session
  ...
  for the next session (S21-S28 pattern). S26 demonstrated that a
...
S28 ran across 3 commits + Phase 2 source-verification + Phase 6
close-out + 1 post-close hygiene fold, comfortably within
context. S29 budget per scope:
...
- Delegate multi-file design-of-record analysis to Explore
  subagent per S22-S28 "Explore-subagent + spot-check" LESSONS
  pattern.

Self-monitor cadence:
- Report estimated context usage at ~30%, ~60%.
- If usage crosses ~70% before the chosen S29 scope closes,
  transition per "no mid-commit-batch transitions" — finish
  in-flight sub-surface, then close session and refill the
  transition template for Session 30.

---

## Reporting in chat at session close

After all Session 29 commits land + push + close-out per the
S13-28 pattern:

1. Commit SHA(s) of each S29 sub-surface.
2. Sub-surface(s) landed (per candidate).
3. Test count delta: 970 (or 480/538/944) baseline → S29 close.
4. Driver suite count at S29 close (52/52 expected unless
...
12. Verify-before-asking summary: any source-verification
    findings surfaced.
13. Outstanding items for Session 30.
14. Tags state at S29 close.
...
All S28 close-out commits (`2bb5879` primary + `85a84c4` anchor
follow-up + `3089faa` post-close hygiene fold) plus the S28
deliverables have been folded directly into this prompt — S29
does not need a separate amendment file:

- **S28 close-out anchors** folded into Step 0.1 (workspace
  anchor `3089faa`; repo anchor `ae9e627` with operator-eval_data-
  commit tolerance), Step 0.5 (Canonical S28-close baseline 970
  pinned with 16-path invocation + sub-totals + breadcrumb note
  for the S28 +1 net-new test location; 944 narrower for
  candidates that don't touch ADLS or robots_gate_integration),
  Step 0.9 (S24+S25+S26+S27 stability PLUS new S28 ShardResult
  field check + `_build_shard_result` behavioral smoke +
  cascade.py Stage 1 invoker AST inspection).
- **2 LESSONS sections from S28 close**:
  - "Empirical-vs-by-design distinction in test pins" referenced
    at Phase 2 "CRITICAL Phase 2 hygiene from S28 LESSONS"
    (apply during retrofit-style closures) AND at the Verify-
    before-asking section's LESSONS-folded-discoveries list.
  - "Phase 0 fixture-count commands need `2>/dev/null` + a
    bounded timeout" referenced at Phase 0 Step 0.4 (use the
    Python pattern) AND at the Verify-before-asking section's
    LESSONS-folded-discoveries list.

If new amendments arise pre-S29 open, walk them per the reviewer-
