# Session 36 prompt — scope picked at Phase 1
# (S35 closed a NEW candidate at repo f80ccdc — fresh live ADLS
#  coverage for the parquet ShardWriter via adlfs [the largest
#  uncovered ADLS *write* surface; a DIFFERENT client stack from the
#  S33/S34 cost-journal SDK tests]. That candidate is now CLOSED. S36
#  enters with the SAME near-empty warm-candidate queue as S33/S34/S35:
#  A blocked / D operator-led / E exhausted. No new carry-forward was
#  introduced by S35 — but the S35 survey NAMED the remaining uncovered
#  adlfs write surfaces, which are now explicit fresh-candidate space.)

**Drafted at Session 35 close (2026-06-02), operator-commissioned.**
Mirrors the S20/.../S34/S35 prompt structure. Scope-agnostic at
...
This prompt should be invoked from `~/crawler-audit/SESSION_36_PROMPT.md`.
...
- **Parquet ShardWriter adlfs live coverage (S35 candidate) — CLOSED
  at S35** (`f80ccdc`). The parquet `ShardWriter` is now live-tested
  against Azurite via adlfs on port 10002
  (`test_parquet_writer_adls_azurite.py`). **NOT a carry-forward.**

**No new carry-forward introduced by S35.** The S35 deliverable was a
single commit (1 new live test file); the canonical 970 test count is
unchanged.

**Scope-availability note (IMPORTANT):** with A blocked, D
operator-led, E exhausted, and the S35 candidate now closed, S36 may
...
E, or (d) commissions a NEW candidate. **The S35 survey made the
fresh-candidate space concrete** — the OTHER uncovered adlfs write
surfaces:
  - `classifier/page_acquisition/page_storage.py::_write_pages_via_fsspec`
    (Stage-2 pages.parquet; same tmp→`fs.mv` adlfs shape, uncovered);
  - `output/parquet_writer.py::PartitionedShardWriter` (the Hive
    `write_to_dataset(filesystem=fs)` path — a DIFFERENT pyarrow code
    path than the S35-covered `ParquetWriter`, still uncovered live);
  - `classifier/llm/prompt_logger.py` (fsspec `wb` single object);
...
Operator chooses at Phase 1 which candidate Session 36 ships.
Each candidate has its own Phase 2 design-gate template.

**Sessions 13-35 precedent:** this session does NOT modify the
...
Does NOT modify the Session 19-35 deliverables. Does NOT modify the
...
S36 candidates: Candidate A would add a NEW `barcada-drift` module
...
before Session 36 starts, walk each flagged item against on-disk
reality at the workspace HEAD and repo HEAD `f80ccdc`, BEFORE applying
any change. Per S19-S35 pattern:
...
(a near-HALT resolved inside Phase 3 by source-verifying the adlfs
behavior, not after). Apply the same discipline: validate any
external-service interaction against the real emulator before
committing claims about it.
...
# Expect: 236a19c (S35 anchor-pin) OR this S36 prompt-drafting commit
# succeeding it OR a later commit if additional workspace doc edits /
# prompt revisions landed. If N commits ahead of 236a19c, verify each
# via `git log --oneline 236a19c..HEAD` against expected
# prompt-finalization / doc-edit patterns; surface the SHA delta and
# request authorization if anything is unexpected. (S20-S35 precedent:
...
# Expect: f80ccdc (S35 WA2.W8.adls-live-parquet-writer).
#
# Tolerated delta: operator-side eval_data labeling commits between
# S35 close and S36 open (Sessions 8-35 precedent). Verify via
# `git show --stat <sha>` for every commit in f80ccdc..HEAD that each
# is strictly within eval_data/* (NO src/barcada_scraper/*, NO tests/*
# [INCLUDING the three Azurite live-test files], NO .github/*
...
### Step 0.2 — Tags (13 expected; unchanged from S30-S35 close)

```
git -C /Users/administrator/projects/barcada-scraper tag -l | sort
# Expect 13 tags (UNCHANGED; S35 placed no tag per its 1.TAG = defer).
...
# at S34 AND S35 despite three live-ADLS surfaces landing. If S36
...
# S29-S35 did NOT touch this surface.
```

### Step 0.4 — Fixture counts (use the Python rglob pattern, NOT bare find)

**UNCHANGED FROM S33/S34/S35**: S35 shipped a live test file, NOT a
...
### Step 0.5 — Test-suite baseline (S36 canonical)
...
#
# Sub-totals (16 paths; ALL identical to S27-S35 close — ALL THREE
...
#
# Pinned in SESSION_LOG.md "Canonical S35-close baseline" block.
```
...
Phase 3 commits in S36. The cumulative-test-count gate never lets it
...
# S35 sub-surface (parquet writer hermetic suite — unchanged by S35; the
# S35 live test is a SEPARATE file under tests/classifier/pipeline/):
.venv/bin/python -m pytest tests/test_parquet_writer.py -q              # expect 33
```

### Step 0.9 — S25-S35 invariants + cassette-dir + S33/S34/S35 live-test presence

**Coverage note**: equals S35's Step 0.9 plus a NEW check for the S35
...
`cat >…<<'PYEOF'` heredocs.** Empirically re-confirmed S33-S35: the
heredoc form for any AST / secrets-referencing script trips the safety
hook even though the script touches no real secret — the hook matches
the heredoc command string, not the file. Re-create the SAME script via
the Write tool and run it via `.venv/bin/python <path>`. Run any
secrets-referencing import check as its OWN Bash call (combining it with
other `python -c` blocks also trips the hook). If a script fails to
STAGE, switch to the Write tool; if it fails to EXECUTE (a real
...
# (a2) S35 parquet writer public API unchanged (inline-safe; the S35 live
# test depends on these public symbols — assert they remain):
.venv/bin/python -c "
import inspect
...
#     tool; run via .venv/bin/python — same as S35's (c)).
#     [Re-use S35's check body verbatim; all 8 slots populate.]
...
#     tool (hook blocks inline ast.parse). Re-use S35's check_s28_ast body.

# (e) S29 K-b smoke script existence + import — own Bash call (secrets).
#     Re-use S35's check_s29_smoke body via the Write tool.

# (f) S31+S32 cassettes (10) + S33 Azurite primitive + S34 race test +
#     S34 CI workflow + S35 parquet adlfs test. Stage
#     check_s36_deliverables.py via the Write tool and run it:
```

Stage the following as `check_s36_deliverables.py` via the Write tool
...
assert '_BLOB_PORT = 10002' in qtext, 'S35 fixture lost its distinct port (collides with S33/S34)'
...
assert 'AzureBlobFileSystem' in qtext, 'S35 test lost its teeth (negative-control assertion)'
assert '--skipApiVersionCheck' in qtext, 'S35 fixture lost the baked version-skew flag'
print('OK (f) S31+S32 cassettes (10) + S33 primitive + S34 race+CI + S35 parquet adlfs test present (posture intact)')
```

If any of 0.1-0.9 fail, HALT before doing any work.

**Optional Docker-gated check (NOT a halt condition)**: if Docker is
available locally, the operator MAY confirm ALL THREE live tests still
pass + coexist:
```
.venv/bin/python -m pytest -m live tests/classifier/pipeline/ -q
# Expect: 3 passed, 209 deselected (needs Docker + the Azurite image;
# SKIPS cleanly if absent). Ports S33=10000 / S34=10001 / S35=10002.
# Informational; Docker is NOT required for S36 unless the chosen scope
# is new live-test work.
```

---

## Required workspace reading (Session 36 first 10 minutes)

In this order:

1. **`~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md`** — full handoff
   state at S35 close. Lists scope candidates (A/D/E) with
   prerequisites + the named fresh-candidate adlfs surfaces. The S35
   candidate is CLOSED; Candidate E is EXHAUSTED at 30. Note the
   explicit near-empty-warm-queue flag.

2. **`~/crawler-audit/SESSION_LOG.md`** Session 35 entry — the ADLS
   write-surface survey; the adlfs-vs-SDK stack split; the parquet
   ShardWriter carve-out; the negative-control teeth check; the
   makedirs→mkdir build-time gotcha; the 970 baseline re-pin.

3. **`~/crawler-audit/LESSONS.md`** — re-read sections relevant to the
   chosen candidate:
   - **Mandatory regardless of candidate**: the S35 fold "adlfs is a
     SEPARATE ADLS write stack from the azure-storage-blob SDK — cover
     it on its own" + the S34 fold "A carve-out claim must be verified
     against the test BODY, not its name/marker".
   - **If a NEW live-service candidate** (another adlfs surface or
     lease/SAS): the S33 fold (live-emulator teardown + version skew) +
     the S35 fold (survey by client stack; teeth = AzureBlobFileSystem
     + blob-relative final_path read back via a FRESH handle; the
     `makedirs`-no-op → `mkdir` gotcha; DISTINCT port — S33=10000,
     S34=10001, S35=10002, so use 10003+) + S30 posture-validation +
     the S24/S29/S30/S33/S34/S35 public-API-only pattern.
   - **If Candidate A**: S29 prereq-audit fold + S22-S35
...
6. For a NEW adlfs-surface candidate, the relevant production module +
   its hermetic test:
   - parquet: `src/barcada_scraper/output/parquet_writer.py` +
     `tests/test_parquet_writer.py` (33 hermetic file:// tests) +
     `tests/classifier/pipeline/test_parquet_writer_adls_azurite.py`
     (the S35 live test — the template to mirror).
   - pages: `src/barcada_scraper/classifier/page_acquisition/page_storage.py`.
...
operator-led / E exhausted / S35-candidate closed; NO self-contained
warm candidate remains; the named adlfs surfaces are FRESH candidates).
S36 may have no clearly-actionable engineering scope without operator
action. Then operator picks one.

### Three invocation shapes (S36 continues the empty-warm-queue condition)

- **Decided before invocation (cleaner).** Operator names the
  candidate when commissioning S36. Phase 1 CONFIRMS rather than
  ...
  adlfs candidate: source-verify the surface is genuinely uncovered);
  proceed to that candidate's Phase 2 gate. If "reopen E", see
  plan-amendment.

- **Decided at Phase 1 (more flexible).** S36 opens, Phase 0 verifies,
...
**Empirical prereq audit at S35 close** (carry-forward): 0 canary_runs
parquets; no barcada/canary/drift plist; no AI/ML responses. Re-run
these empirical checks at S36 Phase 1 BEFORE the candidate-choice
...
### NEW candidate space (now NAMED by the S35 survey)

With the S35 candidate closed, the remaining uncovered adlfs write
surfaces are explicit fresh candidates (each scoped at Phase 1/2 from
first principles, mirroring the S35 live-test shape):

- **page_storage adlfs leg** — `page_storage._write_pages_via_fsspec`
  (Stage-2 pages.parquet; same tmp→`fs.mv` shape). Smallest, most
  S35-symmetric. A live test would mirror S35 on port 10003.
- **PartitionedShardWriter adlfs leg** — the Hive
  `write_to_dataset(filesystem=fs)` path; a DIFFERENT pyarrow code path
  than S35's `ParquetWriter`. Closes the second half of `parquet_writer.py`.
...
- **cost-journal lease/SAS live paths** — the azure-storage-blob SDK
  ...
  the S35 survey found prod's adlfs path uses connection-string /
  managed-identity, NOT a lease — re-verify for the SDK path before
  scoping).
...
LESSONS)**; **survey coverage by CLIENT STACK, not by "is there an
Azurite test" (S35 LESSONS)**; default new external-service-backed
tests to operator-smoke / live-on-demand posture unless a named
carve-out justifies CI (S30); if a new live fixture must coexist with
the S33/S34/S35 fixtures, give it a DISTINCT port + container name
(10000/10001/10002 taken → use 10003+); size new scripts with the
~70-100 LOC additive overhead floor.
...
- **A NEW adlfs-surface candidate completing W A.2**: defer OR
  ...
  (OFFERED at S33, DEFERRED at S33 AND S34 AND S35).
...
the `[[verify-before-asking-discipline]]` AND the S22-S35
...
W4.1.5 driver beyond the S16/S27/S28 exceptions OR to any S19-S35
deliverable (including the S33 Azurite test, the S34 concurrency test,
the S34 CI workflow, the S35 parquet adlfs test, or re-recording/deleting
any of the 30 cassettes) — surface as a design-gate sub-question before
patching.

### CRITICAL Phase 2 hygiene

Carry forward S26/S28/S29/S30/S34/S35 hygiene verbatim: count each Q-*
...
narrow the wording to what the body actually does (S34 fold)**; and
...
**S35 live-test shape** (the closest precedent):
- Live marker, skip-by-default, Docker-gated, self-reproducing fixture
  ...
  baked `--skipApiVersionCheck`, DISTINCT port 10003+ + container name.
- Shared-key auth via the Azurite connection string (precedence #2 of
  `storage_options_from_env`) UNLESS source shows the surface uses a
  different mechanism — verify, do not assume (S34/S35 anti-trap).
...
- Teeth: assert the resolved fs is `AzureBlobFileSystem` AND a
  public-surface signal (e.g. blob-relative path); read the artifact
  back through a FRESH adlfs handle; demonstrate the negative control
  fails on a `file://` fallback.
- Decide explicitly whether it joins the CI workflow's `-m live` dir
  selection (it will be picked up automatically if placed under
  `tests/classifier/pipeline/`) and keep the skip-fail guard meaningful.

### Shared sub-questions (all candidates)

- **Q-SHARED.1 Commit shape**: per-module / per-sub-surface (S18-S35
  default; Recommended) vs single bundled commit (only when genuinely
  one self-contained sub-surface, as the S35 single live test file was).
...
before starting the next. Per S22-S35 precedent. If a mid-sub-surface
dependency emerges, HALT and surface as a design-gate sub-question.

### Build-time validation spike (NEW emphasis from S35)
...
form, the auth mechanism, the resolved-fs type, and a full round-trip
(write → read back via a FRESH handle). This catches version-skew and
API-semantic gotchas (e.g. S35's `makedirs`-no-op) inside Phase 3
...
.venv/bin/python -m pytest \
    ...
    <new S36 test paths if any> -q
# A new @pytest.mark.live test stays OUT of the canonical headline
# unless a deliberate decision adds it (S33/S34/S35 Option-1 posture).
# If it is placed under tests/classifier/pipeline/, it is auto-picked-up
# by the CI workflow's `-m live` selection but NOT by the canonical
# 16-path (which lists files explicitly). Verify a new live test
# out-of-band: `pytest -m live tests/classifier/pipeline/`.
```
...
# `ruff format` once — S35 reformatted a multi-line conn-string + a
# records comprehension. Apply it, re-check, re-run the test.)
```
...
new-file LOC claims use `wc -l` (S29). **For any live test, include a
teeth check** (a negative control demonstrated to FAIL on the wrong
path) in the table, not just "it passes" (S34/S35).
...
(`WA2.W8.*` is the established prefix for this surface family — S35
...
NO backticks in the body (S35 prompt-review fold).
...
S33/S34/S35 pattern (`pytest -m live tests/classifier/pipeline/`).
...
edits to `eval_data/*` can fail the gate even though no S36 commit
...
2. Confirm the S36 commits do not touch eval_data
...
S20+S22 precedent: operator chose (a). S21/S23-S35 did not need this
...
the parquet adlfs leg (S35). The `workstream-a-week2-end` tag was
OFFERED at S33 and DEFERRED at S33 AND S34 AND S35; if S36 declares
W A.2 closed it remains appropriate.

---

## Phase 6 — Workspace close-out

- Append Session 36 entry to `~/crawler-audit/SESSION_LOG.md` including
  a **Canonical S36-close baseline** block with the exact pytest
  invocation + verified count (per S22-S35 LESSONS). **If the fixture
  ...
  `exclusions_count == N` as the S37 Phase 0 Step 0.4 forward note.**
  Otherwise pin `cassette_count == 30` / `exclusions_count == 30`.
- Refill `~/crawler-audit/SESSION_TRANSITION_TEMPLATE.md` for Session
  37 — explicitly pin the S37 Phase 0 workspace anchor SHA per the
  S21-S35 post-audit pattern. After the close-out commit lands, expect
...
Note: drafting the next-session prompt (`SESSION_37_PROMPT.md`) is NOT
a built-in Phase 6 step. Per S20→S35 precedent, prompt-drafting is
operator-commissioned between sessions. If the operator asks for it
explicitly at S36 close, draft it as a separate follow-up.
...
0. Phase 1 carve-out / justification recorded in chat (the surface is
   ...
   the carve-out matched to the actual test body (S34) and a
   demonstrated negative-control teeth check (S35)**.
...
- **S4.** Regression-protection checklist held (see Out-of-scope). In
  particular: ALL S21-S35 deliverables stay at their SHAs; their public
  ...
  10002, AzureBlobFileSystem teeth intact); per-sub-suite counts stay
  green; CRAWLING_POLICY.md 77 lines / 2519 bytes; the combined total
  stays at 970 (or grows) unless a Q-* authorizes a same-shape 1↔1
  replacement.
...
Per the regression-protection checklist. **S35 added one new lock**
(the parquet ShardWriter adlfs live test at `f80ccdc`); otherwise
unchanged from the S35 prompt.
...
- `tests/classifier/pipeline/test_parquet_writer_adls_azurite.py` at
  ...
  `adlfs.AzureBlobFileSystem` (shared-key connection string) on a
  DISTINCT port 10002 / container `barcada-azurite-parquet` (coexists
  with S33=10000 + S34=10001 under one `-m live` run). Teeth: asserts
  the resolved fs is `AzureBlobFileSystem` + a blob-relative
  `final_path`, reads back via a FRESH adlfs handle. Auto-joins the CI
  workflow's `-m live` dir selection. Do NOT modify without Phase 2
  authorization. **`output/parquet_writer.py` is now EMPIRICALLY
  validated against Azurite via adlfs (the S35 test injects only the
  public `storage_options=` seam + `abfss://` URL; the module is
  UNMODIFIED) — keep it that way.** `tests/test_parquet_writer.py` (33
  hermetic file:// tests) remains the default-run guard for the module.
...
expected and unstaged (Sessions 8-35); inter-session operator-side
...
S31+S32+S33+S34+S35 added no new src/ authorizations (S35 was test only).
...
## Verify-before-asking discipline (strict rule from S19-S35)
...
by the test body (S34)**, and **that the negative-control teeth fail on
the wrong path (S35)**. No claims by pattern-completion. Build a
...
- Before claiming a new-file LOC count, run `wc -l` (AFTER any
  ruff-format pass — S35's LOC dropped 317→309 after format).
...
- Before committing, confirm the subject's `WA2.W8.*` workstream label
  reconciles with the decision record (S35 operator-flag).
...
- **adlfs is a SEPARATE ADLS write stack from the azure-storage-blob
  SDK — survey coverage by client stack, not by "is there an Azurite
  test" (S35).** adlfs `makedirs(container, exist_ok=True)` silently
  no-ops; use `mkdir` (catch `FileExistsError`). Teeth for an adlfs
  write test = resolved-fs is `AzureBlobFileSystem` + blob-relative
  `final_path` + read-back via a FRESH handle.
...
- A live-emulator fixture must tear down on setup-phase failure
  ...
  live fixture needs a DISTINCT port + container name (S34/S35:
  10000/10001/10002 taken → 10003+).
- A build-time spike against the real emulator catches API-semantic
  gotchas inside Phase 3 instead of as a post-commit HALT (S35).
...
- Plan-vs-reality at Phase 2 (S22); source-verify drives option-set
  design (S25; S28-S35).
- Test against public API surface only (S24; S29/S30/S33/S34/S35).
...
- Markdown-rendered shell commands may carry NBSP / the env hook blocks
  ...
  the Write tool when exactness matters (S34/S35).
- Operator-driven script LOC: ~70-100 LOC additive overhead floor (S29).
- Empirical Phase 1 prerequisite check before scope-narrow (S29; S30-S35
...
- Reject-cassette cleanup is a two-step asymmetric pattern; `rm`/`find
  ...
  (S31; S32 confirmed; re-confirmed S35: an agent-side `docker rm -f`
  pre-clean tripped the destructive-op hook, but `docker stop` and
  in-fixture `docker rm -f` run fine).

---

## Commit hygiene (per LESSONS + S19-S35 additions)
...
- Commit shape per Q-SHARED.1 (per-module default; single bundled commit
  ...
  as the S35 single live test file was).
...
- For S19-S35-test/fixture/workflow modifications: explicit Phase 2
...
- Workspace close-out (Phase 6) lands as its own commit, followed by
  1-2 follow-up commits pinning the anchor SHA for S37.

---

## Context-window awareness

S35 ran Phase 0 → Phase 6 in a single context window with 1 repo commit
(Phase 0 verify + Phase 1 scope + Phase 2 source-verify survey +
operator-confirmed carve-out + Phase 3 impl with a build-time spike +
close-out), comfortably within budget. S36 budget per scope:

- Candidate A: medium (~370-400 LOC); only if unblocked.
- Candidate D: small; operator-led.
- A NEW adlfs-surface candidate (page_storage / PartitionedShardWriter /
  prompt_logger / lease-SAS): small-to-medium; size at Phase 2; mirror
  the S35 single-live-test shape.
...
- For any live-service-backed test, default to the S33/S34/S35 fixture
  ...
  (10003+) if coexisting; container creation via `mkdir` not
  `makedirs(exist_ok=True)`; keep the test off the canonical headline
...
- If usage crosses ~70% before the chosen S36 scope closes, finish the
  in-flight sub-surface, then close session and refill the transition
  template for Session 37.

---

## Reporting in chat at session close

After all Session 36 commits land + push + close-out per the S13-35
pattern:

1. Commit SHA(s) of each S36 sub-surface.
2. Sub-surface(s) landed (per candidate).
3. Test count delta: 970 baseline → S36 close.
4. Driver suite count at S36 close (52/52 expected).
...
8. Live ADLS coverage status: K-b SHIP S29; K-b EXEC S30; K-a Azurite
   ...
   ShardWriter adlfs leg S35. Report S36's disposition if touched.
9. Any spend (LLM, infrastructure).
10. Verify-before-asking summary: source-verification findings.
11. Outstanding items for Session 37.
12. Tags state at S36 close.
...
All S35 close-out commits (`fd3b5d8` primary + `236a19c` anchor-pin)
plus the S35 disposition have been folded directly into this prompt —
S36 does not need a separate amendment file:

- **S35 close-out anchors** folded into Step 0.1 (workspace anchor =
  `236a19c` + this prompt-drafting commit; repo anchor `f80ccdc` with
  ...
  Step 0.8 (added the parquet hermetic 33-test sub-suite check), Step
  0.9 (added the S35 deliverable presence + posture check via
  `check_s36_deliverables.py` + the (a2) parquet public-API check).
- **1 NEW LESSONS section from S35** ("adlfs is a SEPARATE ADLS write
  stack from the azure-storage-blob SDK — cover it on its own")
  referenced in the Verify-before-asking LESSONS list.
- **S35 candidate CLOSED** (parquet ShardWriter adlfs leg) folded into
  the carry-forward list + Scope section + Out-of-scope's S35
  deliverable (NEW LOCK).
- **Candidate E still EXHAUSTED** (30 reached) — requires a
  plan-ceiling revision first.
- **NEW candidate space NAMED** — the S35 survey's remaining uncovered
  adlfs surfaces (page_storage / PartitionedShardWriter / prompt_logger)
  + lease/SAS cost-journal paths, each a fresh Phase-2 candidate.

If new amendments arise pre-S36 open, walk them per the
